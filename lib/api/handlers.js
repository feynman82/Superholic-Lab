...
async function callGemini(prompt, { temperature = 0.1, maxOutputTokens = 1024 } = {}) {
...
function getCheckoutPriceId(plan, billing) {
  if (billing === 'annual') {
    return { single_subject: process.env.STRIPE_SINGLE_SUBJECT_ANNUAL_PRICE_ID, all_subjects: process.env.STRIPE_ALL_SUBJECTS_ANNUAL_PRICE_ID, family: process.env.STRIPE_FAMILY_ANNUAL_PRICE_ID }[plan] || null;
  }
  return { single_subject: process.env.STRIPE_SINGLE_SUBJECT_PRICE_ID, all_subjects: process.env.STRIPE_ALL_SUBJECTS_PRICE_ID, family: process.env.STRIPE_FAMILY_PRICE_ID }[plan] || null;
}
...
export async function handleGradeAnswer(req, res) {
  ...
  try {
    ...
    try {
      responseText = await callGemini(fullPrompt, { temperature: 0.1, maxOutputTokens: 1024 });
    } catch (geminiErr) {
