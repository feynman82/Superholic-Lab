      ...
      if (user) {
        ...
        if (quest) {
          ...
          if (stepCfg.scaffold_mode === 'socratic') {
            // Fetch student name for personalisation
          ...
          // Return message count so tutor.js can gate "Mark Day 2 Complete"
          questMessageCount = cleanHistory.length;
        }
      }
    }
  } catch (questErr) {
    // Non-fatal: fall back to default prompt
    console.error('[chat] Quest mode setup error:', questErr.message);
  }

  try {
    const payload = { systemInstruction: { parts: [{ text: systemPrompt }] }, contents: cleanHistory, generationConfig: { temperature: 0.4, maxOutputTokens: 1024 } };
  ...
  try {
    ...
    try {
      const raw = await callGemini(
        `You are a creative educational writer for Singapore primary students. Return ONLY valid JSON.\n\n${narrativePrompt}`,
        { temperature: 0.7, maxOutputTokens: 512, responseMimeType: 'application/json' }
      );
...
function computeDayUnlockStatus(dayCompletedAt = {}) {
  ...
  for (let step = 0; step <= 2; step++) {
    if (step === 0) { status[step] = { unlocked: true }; continue; }
    const prevTs = dayCompletedAt[String(step - 1)];
    if (!prevTs) { status[step] = { unlocked: false, reason: 'previous_step_incomplete' }; continue; }
    const unlockAt = midnightSGTAfter(prevTs);
    if (sgNow >= unlockAt) {
      status[step] = { unlocked: true };
    } else {
      status[step] = { unlocked: false, reason: 'midnight_gate', unlocks_at: unlockAt.toISOString() };
    }
  }
  return status;
}
...
async function questsBatch(req, res) {
  ...
  const topic          = stepCfg.topic || quest.topic;
  const dbSubject      = quest.subject.charAt(0).toUpperCase() + quest.subject.slice(1);
  ...
  const QUESTION_COLS = 'id, question_text, options, correct_answer, wrong_explanations, worked_solution, type, difficulty, topic, sub_topic, parts, passage, blanks, passage_lines, keywords, model_answer, progressive_hints, visual_payload, marks, accept_also';
  ...
  const questions = [...questionSets.flat(), ...transferSets.flat()];
  return res.status(200).json({ questions, step_config: stepCfg });
}
