 ...
 *
 * SUBSCRIPTION FLOW:
 *   Signup → free 7-day trial (no Stripe, no card).
 *   Subscribe → Stripe charges immediately (no trial_period_days).
 *   Webhook → updates profiles.subscription_tier in Supabase.
 *
 * REFERRAL FLOW:
 *   Parent shares ?ref=CODE link → referred signs up → on paid subscribe →
 *   Stripe coupon (STRIPE_REFERRAL_COUPON_ID) applied to referrer's next invoice.
 *
 * PAUSE FLOW (monthly only):
 *   Parent schedules pause → stored in profiles.pause_scheduled + pause_resume_at →
 *   invoice.payment_succeeded webhook activates Stripe pause_collection at period end →
 *   auto-resumes at pause_resume_at.
 *
 * ONE-TIME SETUP REQUIRED IN STRIPE DASHBOARD:
 *   1. Create coupon: ID = value of STRIPE_REFERRAL_COUPON_ID env var,
 *      100% off, duration = once.
 *   2. Add webhook event: invoice.payment_succeeded (for pause activation).
 *   3. Add env var STRIPE_REFERRAL_COUPON_ID to .env and Vercel.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// PRICING UTILITIES (shared across handlers)
// MRR figures use Singapore FRS 115: annual recognised at 1/12 per month.
// Stripe fee estimate: 3.4% + S$0.50 per transaction (SG-issued cards).
// ─────────────────────────────────────────────────────────────────────────────
...
function isAnnualPriceId(priceId) {
  return [
    process.env.STRIPE_ALL_SUBJECTS_ANNUAL_PRICE_ID,
    process.env.STRIPE_FAMILY_ANNUAL_PRICE_ID,
  ].filter(Boolean).includes(priceId);
}

// Stripe fee per billing cycle (amortised for annual)
function estimateStripeFee(cycle, grossMonthlyMrr) {
  if (cycle === 'monthly') {
    return grossMonthlyMrr * 0.034 + 0.50;
  }
  // Annual: fee is (annual_price * 3.4% + S$0.50) / 12
...
export async function handleChat(req, res) {
  ...
  if (!process.env.GEMINI_API_KEY) return res.status(500).json({ error: 'AI tutor is not configured.' });
  ...
  if (cleanHistory.length === 0 || cleanHistory[0].role !== 'user') return res.status(400).json({ error: 'First message must be from the user' });

  try {
    const payload = { systemInstruction: { parts: [{ text: OMNI_TUTOR_SYSTEM_PROMPT }] }, contents: cleanHistory, generationConfig: { temperature: 0.4, maxOutputTokens: 1024 } };
...
export async function handleSummarizeChat(req, res) {
  ...
  try {
    ...
    const rawResponse = await callGemini(`${SUMMARIZE_SYSTEM_PROMPT}\n\nCHAT LOG:\n${chatLog}`, { temperature: 0.3, maxOutputTokens: 1000, responseMimeType: 'application/json' });
    ...
    const { error: dbErr } = await adminDb.from('study_notes').insert({ student_id, subject: subject || 'general', topic: topic || 'mixed', title: generatedData.title, content_html: generatedData.content_html, is_read: false });
...
export async function handleCheckout(req, res) {
  ...
  if (!priceId) {
    const cycle = (billing === 'annual' || billing === 'yearly') ? 'annual' : 'monthly';
    console.error(`[checkout] Missing price ID for plan=${plan} billing=${cycle}`);
  ...
  try {
    ...
    const stripeMsg = err?.message || 'Unknown Stripe error';
    console.error('[checkout] Stripe error:', stripeMsg);
    return res.status(500).json({ error: `Stripe error: ${stripeMsg}` });
  }
}
  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        if (!userId) { console.warn('[webhook] No client_reference_id'); break; }
        ...
        // ── Referral credit: apply coupon to referrer if this new subscriber came via referral ──
        if (process.env.STRIPE_REFERRAL_COUPON_ID) {
          ...
          if (referral) {
            ...
            // Get referrer's active subscription to apply coupon
            const { data: referrerSub } = await db.from('subscriptions')
            ...
            if (referrerSub?.stripe_subscription_id) {
              try {
                ...
                // Mark as credited and increment counter
                await db.from('referrals').update({
                  status:          'credited',
                  credited_at:     new Date().toISOString(),
                  stripe_coupon_id: process.env.STRIPE_REFERRAL_COUPON_ID,
                }).eq('id', referral.id);

                const { data: refProfile } = await db.from('profiles')
                  .select('referral_credits_earned')
                  .eq('id', referral.referrer_id)
                  .single();
                await db.from('profiles').update({
                  referral_credits_earned: (refProfile?.referral_credits_earned || 0) + 1,
                }).eq('id', referral.referrer_id);

                console.log(`[webhook] Referral credit applied to ${referral.referrer_id}`);
              } catch (couponErr) {
                console.error('[webhook] Referral coupon error:', couponErr.message);
              }
            }
          }
        }
      ...
      case 'invoice.payment_succeeded': {
        ...
        // Check if pause was scheduled for this profile
        ...
        if (subRow?.profile_id) {
          const { data: profile } = await db.from('profiles')
            .select('pause_scheduled, pause_resume_at')
          ...
          if (profile?.pause_scheduled && profile?.pause_resume_at) {
            ...
            try {
              await stripe.subscriptions.update(subId, {
                pause_collection: { behavior: 'void', resumes_at: resumeTs },
              });
              await db.from('subscriptions').update({
                paused_at:  new Date().toISOString(),
                resumes_at: profile.pause_resume_at,
                status:     'paused',
              }).eq('stripe_subscription_id', subId);
              await db.from('profiles').update({
              ...
              console.log(`[webhook] Pause activated for ${subRow.profile_id}, resumes ${profile.pause_resume_at}`);
            } catch (pauseErr) {
              console.error('[webhook] Pause activation error:', pauseErr.message);
            }
          }
        }

        console.log(`[webhook] Payment succeeded: ${subId}`);
        break;
      }
      ...
      case 'customer.subscription.updated': {
        ...
        if (plan) {
          ...
          if (uid) await db.from('profiles').update({ subscription_tier: plan, max_children: maxChildrenForPlan(plan) }).eq('id', uid);
        }

        // If Stripe signals the pause was removed (auto-resume)
        if (!sub.pause_collection) {
          ...
          if (subRow?.profile_id) {
            await db.from('profiles').update({
              subscription_tier: subRow.plan_name || plan || 'all_subjects',
              pause_scheduled:   false,
        ...
        console.log(`[webhook] Updated: ${sub.id} → ${sub.status}`);
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        console.log(`[webhook] Cancelled: ${sub.id}`);
        break;
      }
...
function buildGenerateUserPrompt(type, count, subject, level, topic, difficulty) {
  const base = `Generate ${count} ${type.replace('_', '-')} question${count > 1 ? 's' : ''} for: Subject: ${subject}, Level: ${level}, Topic: ${topic}, Difficulty: ${difficulty}. All content must be MOE-aligned, use Singapore context.`;
  const extras = {
    mcq:          `\n- 4 options (A-D), wrong-answer misconception explanations, examiner_note.`,
    short_ans:    `\n- Concise answer, accept_also for equivalent forms, step-by-step working.`,
    word_problem: `\n- 2-3 parts with coherent narrative, marking scheme per part, examiner_note.`,
    open_ended:   `\n- Root 'passage' (experiment setup). A 'parts' array of sub-questions using part_type: 'text_box' or 'true_false'. Include 'visual_payload' for diagrams.`,
    cloze:        `\n- 5-8 sentences, 7-10 blanks, correct_answer as exact word string, 4 MCQ options per blank.`,
    editing:      `\n- Exactly 5 underlined words, 4+ grammar categories, each line: line_number, text, underlined_word, has_error, correct_word, explanation.`,
    comprehension:`\n- Root 'passage' (300+ words). A 'parts' array of sub-questions using part_type: 'mcq', 'true_false' (statement, correct_answer, reason_evidence), 'referent', 'sequencing', or 'text_box' (needs rubric and model_answer). Double-escape all SQL quotes.`,
    visual_text:  `\n- Root 'image_url' (placeholder) and 'passage' (flyer text). A 'parts' array of 5-8 'mcq' questions.`,
  };
...
// Note: We route comprehension & visual text through the OPEN_ENDED/MCQ prompts as baselines, enhanced by the extras above
...
const schemas = {
    ...
    open_ended:   `\n- passage: string (experiment context)\n- visual_payload: diagram generation object\n- parts: array of sub-questions with label ('(a)','(b)'), part_type ('text_box'), marks, question, model_answer, rubric, keywords.\n`,
    cloze:        `\n- passage: string with [1],[2] blanks\n- blanks: array with number, options (4), correct_answer (A/B/C/D), explanation\n`,
    editing:      `\n- passage_lines: 3 lines, exactly 1 has_error:true\n  Each: { line_number, text, underlined_word, has_error, correct_word, explanation }\n`,
    comprehension:`\n- passage: string (the story)\n- parts: array of objects with part_type ('mcq', 'true_false', 'text_box'), marks, question/instructions. For true_false include 'items' array. For text_box include 'model_answer' and 'rubric'.\n`,
    visual_text:  `\n- passage: string (flyer text)\n- image_url: string\n- parts: array of 'mcq' objects (question, options, correct_answer, explanation).\n`,
  };
...
export async function handleGenerateQuestion(req, res) {
  ...
  try {
    ...
    let responseText, provider = 'gemini';
    try { responseText = await callGemini(prompt, { temperature: 0.4, maxOutputTokens: 4096 }); }
    catch (e) { provider = 'claude'; responseText = await callClaudeRaw(GQ_SAFETY_PROMPT, buildGQSchemaInstructions(subject, level, topic, type, difficulty, safeCount), { maxTokens: 4096 }); }
...
PRINCIPLES: Award marks for demonstrated understanding. Accept sensible alternatives. For Science check CER. For Maths check method shown. Always return valid JSON.
...
function buildGradingRubric(questionType, subject, studentAnswer, question) {
  const { questionText, workedSolution, keywords, modelAnswer, parts, marks, rubric } = question;
  if (questionType === 'comprehension') {
    return `\nQUESTION (${marks} marks): ${questionText}\n\nMODEL ANSWER: ${modelAnswer||workedSolution||'N/A'}\nGRADING RUBRIC: ${rubric||'Award marks based on semantic similarity and understanding.'}\nSTUDENT ANSWER: ${studentAnswer}\n\nEvaluate the student's answer against the rubric. Total: ${marks}mk`;
  }
  if (questionType === 'open_ended' && subject === 'Science') {
    return `\nQUESTION (${marks} marks): ${questionText}\n\nKEYWORDS: ${(keywords||[]).join(', ')}\nMODEL ANSWER: ${modelAnswer||workedSolution}\nSTUDENT ANSWER: ${studentAnswer}\n\nRUBRIC (CER): Claim 1mk, Evidence 1mk, Reasoning 1+mk, Keywords 0.5mk each cluster. Total: ${marks}mk`;
  }
  if (questionType === 'word_problem' && subject === 'Mathematics') {
    const partsText = (parts||[]).map(p => `${p.label}: ${p.question} → answer: ${p.correct_answer}`).join('\n');
    return `\nQUESTION (${marks} marks): ${questionText}\n\nPARTS:\n${partsText}\n\nWORKED: ${workedSolution}\nSTUDENT: ${studentAnswer}\n\nRUBRIC: 1mk method + 1mk answer per part. Allow arithmetic errors. Total: ${marks}mk`;
  }
  return `\nQUESTION (${marks} marks): ${questionText}\nANSWER KEY: ${workedSolution||modelAnswer}\nKEYWORDS: ${(keywords||[]).join(', ')}\nSTUDENT: ${studentAnswer}\nAward ${marks} marks based on demonstrated understanding.`;
}

export async function handleGradeAnswer(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const { questionId, questionType, questionText, subject, studentAnswer, workedSolution, keywords, modelAnswer, parts, marks, rubric } = req.body || {}; // MODIFIED
    if (!questionType || studentAnswer === undefined || !questionText) return res.status(400).json({ error: 'Missing required fields' });
    if (!['word_problem','open_ended','comprehension','visual_text'].includes(questionType)) return res.status(400).json({ error: 'Invalid questionType' }); // MODIFIED
    if (typeof studentAnswer === 'string' && studentAnswer.trim() === '') return res.status(200).json({ questionId: questionId||'unknown', score: 0, maxScore: parseInt(marks,10)||2, feedback: 'No answer provided.', breakdown: [], gradedBy: 'system' });
    if (typeof studentAnswer !== 'string' || studentAnswer.length > 2000) return res.status(400).json({ error: 'studentAnswer must be a string under 2000 characters' });
    const safeMarks = Math.min(Math.max(1, parseInt(marks,10)||2), 10);
    const rubricPrompt = buildGradingRubric(questionType, subject, studentAnswer, { questionText, workedSolution, keywords, modelAnswer, parts, marks: safeMarks, rubric });
    let responseText, gradedBy = 'claude';
    try { responseText = await callClaudeRaw(GRADER_SYSTEM_PROMPT, rubricPrompt, { maxTokens: 1024, model: 'claude-3-5-sonnet-20241022' }); }
    catch (e) { gradedBy = 'gemini'; responseText = await callGemini(GRADER_SYSTEM_PROMPT + '\n\n' + rubricPrompt, { temperature: 0.1, maxOutputTokens: 1024, responseMimeType: 'application/json' }); }
    const grading = JSON.parse(stripMarkdownFences(responseText));
    if (typeof grading.score !== 'number') throw new Error('Missing score');
    return res.status(200).json({ questionId: questionId||'unknown', score: Math.max(0, Math.min(grading.score, safeMarks)), maxScore: safeMarks, feedback: grading.feedback||'Graded.', breakdown: grading.breakdown||[], gradedBy });
  } catch (err) {
    console.error('[grade-answer] Error:', err.message);
    return res.status(200).json({ questionId: req.body?.questionId||'unknown', score: 0, maxScore: req.body?.marks||2, feedback: 'Could not grade automatically. Compare with worked solution.', breakdown: [], gradedBy: 'fallback' });
  }
}
...
export async function handleGenerateQuest(req, res) {
  ...
  try {
    ...
    if (!student_id || !subject || !level || !topic || trigger_score === undefined || trigger_score === null) return res.status(400).json({ error: 'Missing required fields' });
    const adminDb = getAdminClient();
    ...
    const safeSubject = subject.toLowerCase(), safeLevel = level.toLowerCase().replace(/\s+/g, '-'), safeTopic = topic.toLowerCase().replace(/\s+/g, '-');
    const scoreLabel = trigger_score <= 40 ? 'very low' : trigger_score <= 60 ? 'below average' : 'borderline';
    const userPrompt = `Generate a 3-day remedial quest for ${student.name||'a student'} (${student.level||level}). They scored ${Math.round(trigger_score)}% (${scoreLabel}) on "${topic}" in ${subject}. subject: ${safeSubject} | level: ${safeLevel} | topic: ${safeTopic} | score: ${Math.round(trigger_score)}\nDay 1 action_url MUST BE: /pages/tutor.html?subject=${safeSubject}&level=${safeLevel}&intent=remedial&topic=${safeTopic}&score=${Math.round(trigger_score)}\nCRITICAL: No newlines inside JSON string values.`;
    let questData;
    try {
      const raw = await callGemini(`${QUEST_SYSTEM_PROMPT}\n\nUSER:\n${userPrompt}`, { temperature: 0.4, maxOutputTokens: 1024, responseMimeType: 'application/json' });
      questData = JSON.parse(stripMarkdownFences(raw).replace(/[\n\r]/g, ' '));
    } catch (e) {
      const raw = await callClaudeRaw(QUEST_SYSTEM_PROMPT, userPrompt, { maxTokens: 1024 });
      questData = JSON.parse(stripMarkdownFences(raw).replace(/[\n\r]/g, ' '));
    }
    validateQuestSteps(questData.steps);
    const { data: quest, error: insertErr } = await adminDb.from('remedial_quests').insert({ student_id, subject: safeSubject, level: safeLevel, topic: safeTopic, trigger_score: Math.round(trigger_score), trigger_attempt_id: trigger_attempt_id||null, quest_title: questData.quest_title, steps: questData.steps, current_step: 0, status: 'active' }).select('*').single();
...
const ANALYZE_WEAKNESS_SYSTEM_PROMPT = `You are Miss Wena writing a confidential learning-gap report for a Singapore parent.
TONE: Warm, encouraging, British spelling. 3-4 sentences. Always say "your child".
TASK: Synthesise failed questions into ONE root conceptual gap (deepest cause, not surface description).
HTML: <strong> for key terms, <br><br> between paragraphs, <ul><li> for recommendations (max 3).
OUTPUT — ONLY valid JSON, no markdown: { "analysis": "<HTML>", "quest": { "quest_title": "<60 chars>", "steps": [...] } }`;
...
export async function handleAnalyzeWeakness(req, res) {
  ...
  try {
    ...
    if (stuErr || !student) return res.status(404).json({ error: 'Student not found' });
    if (student.parent_id !== user.id) return res.status(403).json({ error: 'Forbidden' });
    const { data: recentQuizzes } = await adminDb.from('quiz_attempts').select('id, topic, level').eq('student_id', student_id).ilike('subject', subject).order('completed_at', { ascending: false }).limit(10);
    if (!recentQuizzes || recentQuizzes.length === 0) return res.status(200).json({ analysis: `Miss Wena needs more data! Complete a few <strong>${subject}</strong> sessions first.` });
    const { data: wrongQs } = await adminDb.from('question_attempts').select('topic, question_text, answer_chosen, correct_answer, difficulty').in('quiz_attempt_id', recentQuizzes.map(q => q.id)).eq('correct', false).limit(15);
    if (!wrongQs || wrongQs.length === 0) return res.status(200).json({ analysis: `Great news! Your child has <strong>no recent mistakes</strong> in ${subject}. Keep it up!` });
    ...
    const failedSummary = wrongQs.map(q => `- Topic: ${q.topic||'?'} | Difficulty: ${q.difficulty||'?'}\n  Q: ${(q.question_text||'').slice(0,200)}\n  Chose: ${q.answer_chosen||'N/A'} | Correct: ${q.correct_answer||'N/A'}`).join('\n\n');
    const userPrompt = `Analyse ${wrongQs.length} failed ${subject} questions (level: ${safeLevel}).\n\nFAILED:\n${failedSummary}\n\nQUEST CONTEXT: subject: ${safeSubject}, level: ${safeLevel}, topic: ${safeTopic}`;
    const rawResponse = await callGemini(`${ANALYZE_WEAKNESS_SYSTEM_PROMPT}\n\n${userPrompt}`, { temperature: 0.3, maxOutputTokens: 1400, responseMimeType: 'application/json' });
    const result = JSON.parse(stripMarkdownFences(rawResponse));
    let savedQuest = null;
    if (result.quest?.quest_title && Array.isArray(result.quest?.steps) && result.quest.steps.length === 3) {
      const { data: inserted, error: insertErr } = await adminDb.from('remedial_quests').insert({ student_id, subject: safeSubject, level: safeLevel, topic: safeTopic, trigger_score: null, trigger_attempt_id: null, quest_title: result.quest.quest_title, steps: result.quest.steps, current_step: 0, status: 'active' }).select('*').single();
    ...
    return res.status(500).json({ error: err.message || 'Could not generate analysis.' });
  }
}
...
export async function handleGenerateExam(req, res) {
  ...
  try {
    ...
    for (const sec of sections) {
      ...
      let responseText, provider = 'gemini';
      try { responseText = await callGemini(GE_SYSTEM_PROMPT + '\n\n' + buildGEPrompt(sec, subject, level, count), { temperature: 0.45, maxOutputTokens: 4096, responseMimeType: 'application/json' }); }
      catch (e) { provider = 'claude'; responseText = await callClaudeRaw(GE_SYSTEM_PROMPT, buildGEPrompt(sec, subject, level, count), { maxTokens: 4096, model: 'claude-haiku-4-5-20251001' }); }
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLER: /api/admin  (enhanced v2 — returns full CRM data)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleAdmin(req, res) {
  ...
  if (callerErr || !caller || !['admin', 'sub-admin'].includes(caller.role)) {
    return res.status(403).json({ error: 'Forbidden — admin or sub-admin role required' });
  }
  ...
  if (req.method === 'GET') {
    ...
    const [profilesRes, studentsRes, subsRes, recentActivityRes, lastActRes] = await Promise.all([
      adminDb.from('profiles')
        .select('id, email, full_name, role, subscription_tier, stripe_customer_id, trial_ends_at, max_children, created_at, pause_scheduled, pause_resume_at')
        .order('created_at', { ascending: false }),
      adminDb.from('students')
        .select('id, parent_id, name, level, created_at')
        .order('created_at', { ascending: false }),
      adminDb.from('subscriptions')
        .select('id, profile_id, stripe_subscription_id, stripe_price_id, plan_name, billing_cycle, status, current_period_end, paused_at, resumes_at')
    ...
    (subsRes.data || []).forEach(s => {
      ...
      if (breakdown[key]) {
        breakdown[key].count++;
        breakdown[key].gross_mrr += pt.mrr;
        breakdown[key].net_mrr   += pt.mrr - estimateStripeFee(pt.cycle, pt.mrr);
      }
    });

    // Enrich recent activity with student/parent names
    ...
    const recentActivity = (recentActivityRes.data || []).map(a => {
      ...
      return {
        ...
        parent_name:   stu.parentId ? profileEmailMap[stu.parentId] : 'Unknown',
        subject:       a.subject,
        score:         a.score,
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLER: /api/pause
// Pause subscription at end of current billing period (monthly only).
// Annual subscribers are blocked — they must contact support.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handlePause(req, res) {
  ...
  try {
    ...
    const adminDb = getAdminClient();
    const { data: sub } = await adminDb.from('subscriptions')
      .select('stripe_subscription_id, stripe_price_id, billing_cycle, status, current_period_end')
    ...
    // Block annual subscribers — Pause button should be hidden on frontend but double-check server-side
    if (isAnnualPriceId(sub.stripe_price_id) || sub.billing_cycle === 'annual') {
      return res.status(400).json({ error: 'Pause is not available for annual subscriptions. Please contact us.' });
    }
    ...
    if (action === 'schedule') {
      ...
      if (isNaN(resumeDate.getTime()) || resumeDate <= new Date()) {
        return res.status(400).json({ error: 'resumeAt must be a valid future date.' });
      }
      await adminDb.from('profiles').update({
        pause_scheduled: true,
        pause_resume_at: resumeDate.toISOString(),
      }).eq('id', user.id);
      const periodEnd = sub.current_period_end ? new Date(sub.current_period_end) : null;
      return res.status(200).json({
        success:           true,
        pause_activates_at: periodEnd?.toISOString() || null,
        resumes_at:        resumeDate.toISOString(),
        message:           `Your subscription will pause on ${periodEnd?.toLocaleDateString('en-SG')} and resume on ${resumeDate.toLocaleDateString('en-SG')}.`,
      });
    }
    ...
    if (action === 'resume_now') {
      ...
      } catch (e) { console.error('[pause] Stripe resume error:', e.message); }

      const priceInfo = getPriceTypeMap()[sub.stripe_price_id];
      await adminDb.from('profiles').update({
        subscription_tier: priceInfo?.plan || 'all_subjects',
        pause_scheduled:   false,
        pause_resume_at:   null,
      }).eq('id', user.id);
...
// GET  → referral stats for current user (code, URL, counts)
// POST → validate referral code at signup and record the referral
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleReferral(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // Auto-generate code if missing (handles pre-migration users)
    if (!code) {
      code = (user.id.replace(/-/g, '').slice(0, 8)).toUpperCase();
      await adminDb.from('profiles').update({ referral_code: code }).eq('id', user.id);
    }

    const referrals = referralsRes.data || [];
    return res.status(200).json({
      referral_code:    code,
      referral_url:     `https://www.superholiclab.com/pages/signup.html?ref=${code}`,
      credits_earned:   profileRes.data?.referral_credits_earned || 0,
      total_referred:   referrals.length,
      total_subscribed: referrals.filter(r => ['subscribed','credited'].includes(r.status)).length,
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'validate' && referral_code) {
      const { data: referrer } = await adminDb.from('profiles')
      ...
      // Only record if no existing referral for this user
      const { data: existing } = await adminDb.from('referrals')
        .select('id').eq('referred_id', user.id).maybeSingle();
      if (!existing) {
...
// Proxies Plausible Stats API — keeps PLAUSIBLE_API_KEY server-side.
// Admin only.
// ─────────────────────────────────────────────────────────────────────────────
...
// PDPA-compliant full account deletion.
// Cancels Stripe, deletes all data, removes auth record.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleAccountDelete(req, res) {
  ...
  try {
    ...
    if (profile?.stripe_customer_id && process.env.STRIPE_SECRET_KEY) {
      try {
        const stripe = new Stripe(process.env.STRIPE_SECRET_KEY, { apiVersion: '2025-01-27.acacia' });
        const activeSubs = await stripe.subscriptions.list({ customer: profile.stripe_customer_id, status: 'active', limit: 10 });
        for (const s of activeSubs.data) await stripe.subscriptions.cancel(s.id);
      } catch (e) { console.error('[account-delete] Stripe cancel:', e.message); }
    }

    // Get student IDs for cascade deletion
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLER: /api/export
// Returns all user data as a downloadable JSON — PDPA compliance.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleExport(req, res) {
  ...
  try {
    ...
    const [profileRes, subsRes, quizRes, referralRes] = await Promise.all([
      adminDb.from('profiles')
        .select('id, email, full_name, subscription_tier, trial_started_at, trial_ends_at, created_at')
        .eq('id', user.id).single(),
      adminDb.from('subscriptions')
        .select('plan_name, billing_cycle, status, current_period_start, current_period_end')
        .eq('profile_id', user.id),
      studentIds.length > 0
        ? adminDb.from('quiz_attempts')
            .select('subject, level, topic, score, total_questions, completed_at')
            .in('student_id', studentIds)
            .order('completed_at', { ascending: false })
            .limit(500)
        : Promise.resolve({ data: [] }),
      adminDb.from('referrals')
        .select('status, referred_at, subscribed_at, credited_at')
        .eq('referrer_id', user.id),
    ]);

    const exportData = {
      exported_at:   new Date().toISOString(),
      pdpa_notice:   'This export contains all personal data Superholic Lab holds about your account, in compliance with the Singapore Personal Data Protection Act (PDPA).',
      profile:       profileRes.data,
      students:      students || [],
      subscriptions: subsRes.data || [],
      quiz_history:  quizRes.data || [],
...
// Inline edit of parent name/email and student name/level.
// Email changes trigger Supabase Auth confirmation email to the new address.
...
export async function handleAdminEdit(req, res) {
  ...
  if (action === 'update_parent') {
    ...
    // Update Supabase Auth email — sends confirmation email to new address (not a silent override)
    if (email?.trim()) {
      const { error: authErr } = await adminDb.auth.admin.updateUserById(userId, { email: email.trim() });
      if (authErr) {
        console.error('[admin-edit] Auth email update error:', authErr.message);
        // Profile table was updated; Auth update is best-effort
      }
    }
