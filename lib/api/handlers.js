...
export async function handleAdmin(req, res) {
  ...
  if (req.method === 'GET') {
    const [profilesRes, studentsRes, subsRes, recentActivityRes, lastActRes, contactPendingRes] = await Promise.all([
      ...
      adminDb.from('contact_messages').select('id').eq('status', 'unread'),
    ]);
    ...
    return res.status(200).json({
      ...
      contact_pending_count:   (contactPendingRes.data || []).length,
    });
  }
...
// Public endpoint — no auth required. Server-side validates all fields.
// Admin reads via /api/admin (service role). contact_pending_count in CRM data.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleContact(req, res) {
  ...
  try {
    ...
    const { error } = await adminDb.from('contact_messages').insert({
      ...
      topic:   (topic || 'General Inquiry').slice(0, 100),
      message: message.trim().slice(0, 5000),
    });
    if (error) throw error;
    console.log(`[contact] New message from ${email.trim()}, topic: ${topic || 'General Inquiry'}`);
    return res.status(200).json({ success: true });
  } catch (err) {
