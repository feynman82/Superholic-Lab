...
async function questsBatch(req, res) {
  ...
  // v3.1 (2026-04-27): added `instructions` so synthesis questions in quest mode
  // receive the per-question instructions text needed to extract the connector word
  // (e.g. 'even though') for the synthesis blueprint UI in quiz.js.
  const QUESTION_COLS = 'id, question_text, instructions, options, correct_answer, wrong_explanations, worked_solution, type, difficulty, topic, sub_topic, parts, passage, blanks, passage_lines, keywords, model_answer, progressive_hints, visual_payload, marks, accept_also';
