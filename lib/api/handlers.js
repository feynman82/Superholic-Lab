...
function getPriceToPlan() {
  ...
  return Object.fromEntries(Object.values(prices).flatMap(plans => Object.entries(plans).filter(([, id]) => !!id).map(([name, id]) => [id, name])));
}
...
export async function handleCheckout(req, res) {
  ...
  try {
    ...
    // IMPORTANT: No trial_period_days here.
    // The free trial is tracked app-side in profiles.trial_ends_at.
    // When a parent clicks Subscribe they are charged immediately.
    // Stripe Checkout presents all payment methods from your Dashboard settings.
    const sessionParams = {
      mode:               'subscription',
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        console.log(`[webhook] Activated: ${planName} for ${userId}`);
        break;
      }
...
export async function handleGenerate(req, res) {
  ...
  // Note: We route comprehension through the OPEN_ENDED prompt as a baseline, enhanced by the extras above
...
function buildGradingRubric(questionType, subject, studentAnswer, question) {
  const { questionText, workedSolution, keywords, modelAnswer, parts, marks, rubric } = question; // ADDED rubric here
  if (questionType === 'comprehension') {
    return `\nQUESTION (${marks} marks): ${questionText}\n\nMODEL ANSWER: ${modelAnswer||workedSolution||'N/A'}\nGRADING RUBRIC: ${rubric||'Award marks based on semantic similarity and understanding.'}\nSTUDENT ANSWER: ${studentAnswer}\n\nEvaluate the student's answer against the rubric. It does not need to be an exact string match. Total: ${marks}mk`;
  }
...
export async function handleGradeAnswer(req, res) {
  ...
  try {
    ...
    if (!['word_problem','open_ended','comprehension'].includes(questionType)) return res.status(400).json({ error: 'Invalid questionType' }); // MODIFIED
    let isEmpty = typeof studentAnswer === 'string' && studentAnswer.trim() === '';
    if (isEmpty) return res.status(200).json({ questionId: questionId||'unknown', score: 0, maxScore: parseInt(marks,10)||2, feedback: 'No answer provided.', breakdown: [], gradedBy: 'system' });
    ...
    const rubricPrompt = buildGradingRubric(questionType, subject, studentAnswer, { questionText, workedSolution, keywords, modelAnswer, parts, marks: safeMarks, rubric }); // MODIFIED
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLER: /api/admin
// Master admin endpoint — gated by role = 'admin' or 'sub-admin'.
// GET  → returns all profiles + students for the admin panel.
// POST → action = 'update_tier' | 'delete_user'
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleAdmin(req, res) {
  ...
  // Validate admin role server-side — never trust the client
  ...
  // ── GET: list all profiles and students ──
  if (req.method === 'GET') {
    const [profilesRes, studentsRes] = await Promise.all([
      adminDb.from('profiles').select('id, email, full_name, role, subscription_tier, stripe_customer_id, trial_ends_at, max_children, created_at').order('created_at', { ascending: false }),
      adminDb.from('students').select('id, parent_id, name, level, created_at').order('created_at', { ascending: false }),
    ]);
    return res.status(200).json({ profiles: profilesRes.data || [], students: studentsRes.data || [] });
  }
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'update_tier') {
      ...
      const validTiers = ['trial', 'all_subjects', 'family', 'single_subject'];
      ...
      if (error) { console.error('[admin] update_tier:', error.message); return res.status(500).json({ error: error.message }); }
      console.log(`[admin] ${user.id} (${caller.role}) set ${userId} → ${tier}`);
      return res.status(200).json({ success: true });
    }
    ...
    if (action === 'delete_user') {
      ...
      if (error) { console.error('[admin] delete_user:', error.message); return res.status(500).json({ error: error.message }); }
