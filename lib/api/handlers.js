  ...
  try {
    ...
    const openAiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: `You are an expert Singapore MOE PSLE Examiner. Grade the student's answer strictly based on the provided Model Answer/Worked Solution. Output ONLY a JSON object with this exact structure: {"score": <integer between 0 and max marks>, "feedback": "<1-2 short sentences of constructive feedback explaining the score>"}. Do not include markdown formatting.` },
...
export async function handleGenerateQuest(req, res) {
  ...
  try {
    ...
    const deterministicSteps = [
      { day: 1, title: 'Diagnostic Briefing', type: 'tutor', action_url: `/pages/tutor.html?subject=${safeSubject}&level=${safeLevel}&intent=remedial&topic=${safeTopic}`, status: 'pending' },
      { day: 2, title: 'Foundation Training', type: 'quiz', action_url: `/pages/quiz.html?subject=${safeSubject}&level=${safeLevel}&topic=${safeTopic}&difficulty=Foundation`, status: 'locked' },
      { day: 3, title: 'Mastery Trial',       type: 'quiz', action_url: `/pages/quiz.html?subject=${safeSubject}&level=${safeLevel}&topic=${safeTopic}&difficulty=Standard`, status: 'locked' },
    ];

    const narrativePrompt = `Generate a fun, kid-friendly narrative for a 3-day remedial learning quest.\nStudent: ${student.name || 'a student'} (${student.level || level})\nSubject: ${subject}\nTopic: ${topic}\nScore: ${Math.round(trigger_score)}%\n\nReturn ONLY a valid JSON object:\n{\n  "quest_title": "Creative Title",\n  "day_1_desc": "Brief engaging description for Day 1",\n  "day_2_desc": "Brief engaging description for Day 2",\n  "day_3_desc": "Brief engaging description for Day 3"\n}`;
...
export async function handleAdmin(req, res) {
  ...
  if (req.method === 'GET') {
    const [profilesRes, studentsRes, subsRes, recentActivityRes, lastActRes, contactRes, qaRes] = await Promise.all([
      ...
      // QA queue count for the admin pulse card
      adminDb.from('question_bank').select('id', { count: 'exact', head: true }).eq('is_ai_cloned', true),
    ]);
...
export async function handleQAQuestions(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    let query = adminDb
      .from('question_bank')
      .select(
        'id, subject, level, topic, sub_topic, difficulty, type, marks, cognitive_skill,' +
        ' question_text, correct_answer, worked_solution, options, parts, wrong_explanations,' +
        ' passage, passage_lines, blanks, is_ai_cloned, created_at, updated_at',
        { count: 'exact' }
      )
      .eq('is_ai_cloned', mode === 'pending')
  ...
  if (req.method === 'PUT') {
    ...
    // approve=true  → publish (is_ai_cloned = false, question goes live)
    // approve=false → send back to queue (is_ai_cloned = true)
    // approve omitted → Save Draft — leave is_ai_cloned unchanged
    if (approve === true)  updates.is_ai_cloned = false;
    if (approve === false) updates.is_ai_cloned = true;
