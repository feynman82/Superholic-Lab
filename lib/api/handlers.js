...
async function callGemini(prompt, { temperature = 0.1, maxOutputTokens = 1024, responseMimeType } = {}) {
  ...
  // FIX: Force Gemini to return valid JSON using strict camelCase for REST API
  if (responseMimeType) {
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLER: /api/analyze-weakness (Deep Dive Dashboard Analysis)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleAnalyzeWeakness(req, res) {
  ...
  try {
    ...
    if (!student_id || !subject) return res.status(400).json({ error: 'Missing student_id or subject' });

    const adminDb = getAdminClient();

    // SECURITY CHECK: Ensure this student actually belongs to the logged-in parent
    const { data: studentCheck, error: stuErr } = await adminDb
      .from('students')
      .select('id')
      .eq('id', student_id)
    ...
    if (stuErr || !studentCheck) {
      return res.status(403).json({ error: 'Forbidden: You do not have access to this student.' });
    }
    
    // Fetch the 5 most recent WRONG questions for this student and subject
    const { data: wrongQs, error } = await adminDb
      .from('question_attempts')
      .select('question_text, answer_chosen, correct_answer, topic, quiz_attempts!inner(subject)')
      .eq('student_id', student_id)
      .eq('correct', false)
      .eq('quiz_attempts.subject', subject.toLowerCase())
      .order('created_at', { ascending: false })
      .limit(5);

    if (error) {
      console.error('[analyze-weakness] DB Error:', error.message);
      throw new Error('Database error');
    }

    if (!wrongQs || wrongQs.length === 0) {
      return res.status(200).json({ analysis: "Excellent work! The student has no recent mistakes recorded for this subject. Keep up the great practice!" });
    }
    // Build the Prompt for Miss Wena
    const prompt = `You are Miss Wena, a highly experienced Singapore MOE primary school tutor. 
    Analyze these recently failed ${subject} questions for a student to find the common underlying conceptual gap.
    
    Failed Questions:
    ${wrongQs.map(q => `- Topic: ${q.topic}\n  Question: ${q.question_text}\n  Student Answer: ${q.answer_chosen}\n  Correct Answer: ${q.correct_answer}`).join('\n\n')}
    
    Task:
    Identify the specific underlying conceptual gap or misconception. 
    Speak directly to the parent (e.g., "Your child seems to be struggling with..."). 
    Keep your advice to exactly 2 or 3 friendly, encouraging sentences. Give one specific actionable tip.
    
    Return ONLY a JSON object in this exact format: { "analysis": "<your text here>" }`;

    // Call Gemini (Forces strict JSON output)
    const responseText = await callGemini(prompt, { temperature: 0.3, maxOutputTokens: 500, responseMimeType: "application/json" });
    
    // Safely parse the response, stripping out any markdown artifacts
    const cleaned = responseText.replace(/^\x60\x60\x60(?:json)?\n?/m, '').replace(/\n?\x60\x60\x60$/m, '').trim();
    const result = JSON.parse(cleaned);

    return res.status(200).json({ analysis: result.analysis });

  } catch (err) {
    console.error('[analyze-weakness] Error:', err.message);
    return res.status(500).json({ error: 'Could not generate deep dive analysis at this time.' });
  }
}
