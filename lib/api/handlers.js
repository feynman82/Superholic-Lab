...
export async function handleAnalyzeWeakness(req, res) {
  try {
    ...
    // Safely grab Supabase environment variables
    ...
    // 2. Fetch the student's recent quiz attempts for this specific subject
    const { data: recentQuizzes, error: quizErr } = await supabase
      .from('quiz_attempts')
      .select('id')
      .eq('student_id', student_id)
    ...
    if (!recentQuizzes || recentQuizzes.length === 0) {
      return res.status(200).json({ analysis: `Miss Wena needs a bit more data! Complete a few more ${subject} practices so I can give you a personalized deep dive.` });
    }

    const quizIds = recentQuizzes.map(q => q.id);

    // 3. Fetch the actual wrong questions linked to those recent quizzes
    const { data: wrongQs, error: qErr } = await supabase
      .from('question_attempts')
      .select('topic, question_text, answer_chosen, correct_answer')
    ...
    // Graceful handling if they got a perfect score recently!
    if (!wrongQs || wrongQs.length === 0) {
      return res.status(200).json({ analysis: `Great job! You haven't made any recent mistakes in ${subject}. Keep up the excellent work!` });
    }

    // 4. Build the Prompt for Miss Wena
    ...
    // 6. Safely parse the response (strips markdown code blocks if Gemini accidentally includes them)
    ...
    // Send the actual error message back to the frontend so we can debug if it fails again
