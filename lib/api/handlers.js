...
export async function runCognitiveDiagnosis(studentId, subject) {
  ...
  if (!weakestTopic) {
    ...
    const lead = al_band ? `${name} is at ${al_band} in ${subjectDisplay}` : `${name} is making early progress in ${subjectDisplay}`;
    return {
      al_band,
      weakest_topic: null,
      weakest_sub_topic: null,
  ...
  const lead            = al_band
    ...
    : `${name} is making early progress in ${subjectDisplay}`;
...
export async function handleRecentAttempts(req, res) {
  ...
  try {
    ...
    // Stats over the last 50 attempts in this cell (caps the read so power
    // users don't pay full-history cost; "lifetime accuracy" can be added
    // later via a SQL aggregate RPC if it becomes a parent-asked question).
    let statsQ = adminDb
      .from('question_attempts')
      .select('correct')
      .eq('student_id', studentId)
    ...
    const totalAttempts = (statsRows || []).length;
