...
import { buildQuestSteps, SYLLABUS_DEPENDENCIES, getPrerequisites, getSubTopics, selectPedagogyMode } from './quest-pedagogy.js';
import {
  xpToLevel, xpInCurrentLevel, xpNeededForNextLevel, levelToRank,
  evaluateLevelUp, evaluateBadges,
} from './badge-engine.js';
import {
  loadPlaybook as loadServerPlaybook,
  getCell as getServerCell,
  getFallbackCell as getServerFallbackCell
} from './wena-playbook-server.js';
import { buildWenaSystemPrompt, PROMPT_VERSION as WENA_PROMPT_VERSION } from './wena-prompts.js';
import { consecutiveStalls } from '../../public/js/wena-stall-detector.js';
import {
  logPedagogyEvent,
  classifyCellHit,
  computeKeywordOverlap
} from './pedagogy-telemetry.js';
...
// ─── EMAIL HELPER ─────────────────────────────────────────────────────────────
// Lightweight Resend wrapper. Used for transactional emails (auto-ack, internal
// alerts). No SDK dependency — Resend's REST API is one fetch call.
//
// Required env vars in Vercel:
//   RESEND_API_KEY      — from https://resend.com/api-keys
//   RESEND_FROM_EMAIL   — e.g. "Superholic Lab <support@superholiclab.com>"
//   SUPPORT_INBOX_EMAIL — internal alert recipient, e.g. "support@superholiclab.com"
//
// Returns { ok: boolean, id?: string, error?: string }. Caller decides whether
// to fail-loud or fail-silent. For contact form, we fail-silent — the row is
// already in Supabase, the email is a nice-to-have.
async function sendEmail({ to, subject, html, replyTo }) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.RESEND_FROM_EMAIL || 'Superholic Lab <support@superholiclab.com>',
    ...
    if (!res.ok) {
      ...
      return { ok: false, error: data.message || 'resend_failed' };
    }
...
// Multi-turn variant — accepts a pre-built messages array (role/content pairs incl. system).
async function callOpenAIMultiTurn(model, messagesArray, { temperature = 0.3, maxTokens = 1024 } = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error('OPENAI_API_KEY not set');
  const res = await fetchWithRetry('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
    body: JSON.stringify({ model, messages: messagesArray, max_tokens: maxTokens, temperature }),
  });
  if (!res.ok) throw new Error(`OpenAI ${res.status}: ${(await res.text()).slice(0, 200)}`);
  const data = await res.json();
  const text = data?.choices?.[0]?.message?.content;
...
// Pass messagesArray for multi-turn chat (OpenAI only); falls back to system+user fold for other providers.
async function callAI(task, { systemPrompt = '', userPrompt = '', messagesArray = null, temperature = 0.3, maxTokens = 1024, responseFormat = null } = {}) {
  const cfg = AI_ROUTING[task];
  if (!cfg) throw new Error(`callAI: unknown task '${task}'`);
  const { provider, model } = cfg;

  if (provider === 'openai') {
    if (messagesArray) return callOpenAIMultiTurn(model, messagesArray, { temperature, maxTokens });
    return callOpenAI(model, systemPrompt, userPrompt, { temperature, maxTokens, responseFormat });
  }
  if (provider === 'anthropic') {
    if (messagesArray) {
      // Fold multi-turn into single system+user for Anthropic (provider fallback only)
      const sys  = messagesArray.find(m => m.role === 'system')?.content || systemPrompt;
      const last = [...messagesArray].reverse().find(m => m.role === 'user');
      return callClaudeRaw(sys, last?.content || userPrompt, { maxTokens, model });
    }
  ...
  if (provider === 'gemini') {
    if (messagesArray) {
      const sys    = messagesArray.find(m => m.role === 'system')?.content || systemPrompt;
      const folded = messagesArray.filter(m => m.role !== 'system')
        .map(m => `${m.role === 'assistant' ? 'Miss Wena' : 'Student'}: ${m.content || ''}`)
        .join('\n\n');
      return callGemini(sys ? `${sys}\n\n${folded}` : folded, { temperature, maxOutputTokens: maxTokens });
    }
...
// Validates AI-generated questions against the canon_subjects/topics/sub_topics
// reference tables (migration 021). DB FK already blocks bad subject + topic at
// INSERT time; this function adds the sub_topic check that the DB does not yet
// enforce, plus catches all three pre-insert with clean error messages.

let _canonCache     = null;
let _canonCacheTime = 0;
const CANON_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

async function loadCanonCache(adminDb) {
  const now = Date.now();
  if (_canonCache && (now - _canonCacheTime) < CANON_CACHE_TTL_MS) return _canonCache;
  const [topicsRes, subTopicsRes] = await Promise.all([
    adminDb.from('canon_topics').select('subject, topic'),
    adminDb.from('canon_sub_topics').select('subject, topic, sub_topic'),
  ]);
  if (topicsRes.error)    throw new Error(`canon_topics fetch failed: ${topicsRes.error.message}`);
  if (subTopicsRes.error) throw new Error(`canon_sub_topics fetch failed: ${subTopicsRes.error.message}`);
  _canonCache = {
    topics:    new Set((topicsRes.data    || []).map(r => `${r.subject}|${r.topic}`)),
    subTopics: new Set((subTopicsRes.data || []).map(r => `${r.subject}|${r.topic}|${r.sub_topic}`)),
  };
  _canonCacheTime = now;
  return _canonCache;
}

function validateAgainstCanon(canon, q) {
  const subject   = q && q.subject;
  const topic     = q && q.topic;
  const sub_topic = q && q.sub_topic;
  if (!subject || !topic) {
    return { ok: false, reason: 'Missing subject or topic' };
  }
  if (!canon.topics.has(`${subject}|${topic}`)) {
    return { ok: false, reason: `Off-canon (subject, topic): "${subject}" / "${topic}"` };
  }
  if (sub_topic && String(sub_topic).trim() !== '') {
    if (!canon.subTopics.has(`${subject}|${topic}|${sub_topic}`)) {
      return { ok: false, reason: `Off-canon sub_topic for ${subject}/${topic}: "${sub_topic}"` };
    }
  }
  return { ok: true };
}

// Filter an array of question candidates against the canon. Returns
// { accepted, rejected } where rejected has { question, reason } pairs.
async function filterByCanon(adminDb, questions) {
  const canon    = await loadCanonCache(adminDb);
  const accepted = [];
  const rejected = [];
  for (const q of questions || []) {
    const v = validateAgainstCanon(canon, q);
    if (v.ok) accepted.push(q);
    else      rejected.push({ id: q && q.id, subject: q && q.subject, topic: q && q.topic, sub_topic: q && q.sub_topic, reason: v.reason });
  }
...
// ── Wena RAP normalisation helpers ───────────────────────────────────────────
// The frontend sends `level` as the raw students.level value ("Primary 5") and
// `subject` as a slug-or-full-name ("english", "Mathematics"). Both are
// canonicalised before retrieval so getCell()/getFallbackCell() see the keys
// they expect. Returns null for shapes outside the canon — callers gate on null.
function _normaliseLevel(raw) {
  if (typeof raw !== 'string') return null;
  const m = raw.replace(/primary\s*/i, 'P').toUpperCase().match(/^P([1-6])$/);
  return m ? `P${m[1]}` : null;
}
function _normaliseSubject(raw) {
  if (typeof raw !== 'string') return null;
  const t = raw.trim().toLowerCase();
  if (t === 'english')                             return 'English';
  if (t === 'mathematics' || t === 'maths' || t === 'math') return 'Mathematics';
  if (t === 'science')                             return 'Science';
  return null;
}

// Sprint 4 canary rollout. Deterministic per-student bucketing so a given
// student is consistently in or out of the canary across sessions.
//   WENA_RAP_ROLLOUT_PCT = 10 → ~10% of students (by id-hash) get RAP
//   WENA_RAP_ROLLOUT_PCT = 100 → all students
//   unset / 0 / NaN → no canary (env flag is the only path)
// `WENA_RAP_ENABLED=true` overrides this and turns RAP on for everyone.
function _isInRapCanary(studentId) {
  const raw = process.env.WENA_RAP_ROLLOUT_PCT;
  if (raw === undefined || raw === null || raw === '') return false;
  const pct = parseInt(raw, 10);
  if (Number.isNaN(pct) || pct <= 0) return false;
  if (pct >= 100) return true;
  if (!studentId || typeof studentId !== 'string') return false;
  // Stable char-sum hash. UUIDs (a-f, 0-9, dashes) produce ~uniform
  // distribution at the 10% bucket-resolution we need.
  let hash = 0;
  for (let i = 0; i < studentId.length; i++) hash += studentId.charCodeAt(i);
  return (hash % 100) < pct;
}

// Sprint 4 CHECK_RESPONSE cross-turn detection. Returns true when the
// previous assistant turn (the one immediately before the current user
// message) contained the cell's check_for_understanding.question — i.e.,
// the student is answering it now.
function _detectCheckResponseTurn(history, cell) {
  if (!cell?.check_for_understanding?.question) return false;
  const lastAsst = [...(history || [])].reverse().find(m => m.role === 'assistant');
  if (!lastAsst || !lastAsst.content) return false;
  const norm = (s) => String(s || '').toLowerCase().replace(/\s+/g, ' ').trim();
  return norm(lastAsst.content).includes(norm(cell.check_for_understanding.question));
}
...
export async function handleChat(req, res) {
  ...
  let rawMessages = messages.filter(m => m.role === 'user' || m.role === 'assistant').slice(-MAX_HISTORY_MESSAGES);

  // Defensive: drop any leading non-user messages after slicing.
  // MAX_HISTORY_MESSAGES truncation can land mid-conversation on an assistant
  // turn, which both OpenAI and Anthropic APIs reject. Strip until the first
  // message is role:user so the array is always API-valid.
  const droppedCount = rawMessages.findIndex(m => m.role === 'user');
  if (droppedCount > 0) {
    console.warn(`[chat] history slice landed on assistant turn — dropped ${droppedCount} leading message(s) to keep array API-valid`);
    rawMessages = rawMessages.slice(droppedCount);
  } else if (droppedCount === -1) {
    // No user messages anywhere in the kept window — genuinely malformed.
    return res.status(400).json({ error: 'No user messages in conversation history' });
  }
  ...
  // ── Wena RAP (Sprint 3 + Sprint 4 + Sprint 8b) ────────────────────────────
  // Flag- or canary-gated. Replaces the system prompt for English chats with a
  // mode-driven template assembled from the playbook cell at (level, topic,
  // sub_topic). Any failure falls back silently to OMNI_TUTOR_SYSTEM_PROMPT —
  // RAP must NEVER surface errors to the student.
  //
  // Sprint 8b architectural shift: for English chats with valid curriculum
  // coordinates, RAP takes precedence over Quest Day 2 Socratic prompt. The
  // playbook cell delivers MOE-aligned, Singapore-context teaching tied to
  // (level, topic, sub_topic) — strictly better than the generic Socratic
  // quest prompt for English. Quest mode wrapping (from_quest URL param,
  // quest_message_count tracking) is preserved.
  //
  // Maths/Science Day 2 still uses buildSocraticQuestPrompt because: Maths
  // has no playbook cells; Science RAP is not yet flag-enabled (Sprint 9).

  // Pull all RAP-relevant fields once so the telemetry write at the end can
  // see them whether or not RAP fired this turn.
  const {
    subject:  rapSubject,
    level:    rapRawLevel,
    currentQuestion: rapCurrentQuestion,
    studentName:     rapStudentName,
    lastCheckQuestion: rapLastCheckQuestion,
    studentId:    rapStudentId,
    parentId:     rapParentId,
    conversationId: rapConversationId,
    lastWenaMode: rapLastWenaMode
  } = req.body || {};

  const RAP_ENABLED = (process.env.WENA_RAP_ENABLED === 'true') || _isInRapCanary(rapStudentId);

  // Hoisted so the telemetry call at the end can see the resolved values.
  let wenaPromptVersion = null;
  let wenaMode          = null;
  let wenaCell          = null;
  let wenaCellHitKind   = 'n/a';
  let wenaRapFired      = false;
  let wenaQuestOverride = false;
  const rapTopicIn      = rapCurrentQuestion?.topic    || null;
  const rapSubTopicIn   = rapCurrentQuestion?.sub_topic || null;
  let wenaFallbackReason = null;

  const subjectN = _normaliseSubject(rapSubject);
  const levelN   = _normaliseLevel(rapRawLevel);
  const englishRapEligible = (
    RAP_ENABLED && subjectN === 'English' && levelN && rapTopicIn
  );

  if (englishRapEligible) {
    // Sprint 8b: RAP runs regardless of whether quest mode set a Socratic
    // prompt above. For English, curriculum coordinates beat quest context.
    try {
      await loadServerPlaybook();
      let cell = rapSubTopicIn ? getServerCell(levelN, rapTopicIn, rapSubTopicIn) : null;
      if (!cell) cell = getServerFallbackCell(levelN, rapTopicIn);

      const baseMode = selectPedagogyMode(rawMessages, cell);
      let mode = baseMode;
      if (rapLastWenaMode === 'DIRECT_TEACH' && cell && _detectCheckResponseTurn(rawMessages, cell)) {
        mode = 'CHECK_RESPONSE';
      }

      const lastUserMsg = [...rawMessages].reverse().find(m => m.role === 'user')?.content || '';
      const checkQ = (mode === 'CHECK_RESPONSE')
        ? (cell?.check_for_understanding?.question || null)
        : (rapLastCheckQuestion || null);

      const newPrompt = buildWenaSystemPrompt({
        mode,
        cell,
        level: levelN,
        studentName: rapStudentName || null,
        lastUserMessage: lastUserMsg,
        checkQuestionAsked: checkQ
      });

      // Commit RAP state only AFTER buildWenaSystemPrompt succeeds.
      // OVERRIDES quest Socratic prompt for English.
      systemPrompt        = newPrompt;
      wenaPromptVersion   = WENA_PROMPT_VERSION;
      wenaMode            = mode;
      wenaCell            = cell;
      wenaCellHitKind     = classifyCellHit(rapTopicIn, rapSubTopicIn, cell);
      wenaRapFired        = true;
      wenaQuestOverride   = (questMessageCount !== null);

      console.log(
        `[wena-rap] mode=${mode} level=${levelN} topic=${rapTopicIn} sub_topic=${rapSubTopicIn || 'n/a'} cell=${cell ? wenaCellHitKind : 'no'} version=${WENA_PROMPT_VERSION} quest_override=${wenaQuestOverride}`
      );
    } catch (rapErr) {
      console.error('[wena-rap] prompt-assembly error, falling back to legacy:', rapErr?.message || rapErr);
      // Defensive reset: if quest mode had set the Socratic prompt, we still
      // fall to legacy OMNI for English consistency (NOT back to the quest
      // Socratic prompt — RAP is the canonical English path even on failure).
      systemPrompt        = OMNI_TUTOR_SYSTEM_PROMPT;
      wenaPromptVersion   = null;
      wenaMode            = null;
      wenaCell            = null;
      wenaCellHitKind     = 'n/a';
      wenaRapFired        = false;
      wenaQuestOverride   = false;
      wenaFallbackReason  = 'rap_error';
    }
  } else if (RAP_ENABLED && systemPrompt === OMNI_TUTOR_SYSTEM_PROMPT) {
    // Non-English RAP-enabled chats — set fallback reason for telemetry.
    if (subjectN === 'English') {
      wenaFallbackReason = 'missing_coordinates';
      console.warn(
        `[wena-rap] Missing curriculum coordinates (level=${levelN || 'null'}, topic=${rapTopicIn || 'null'}) — falling back to legacy prompt`
      );
    } else {
      wenaFallbackReason = 'non_english_subject';
    }
  } else if (!RAP_ENABLED) {
    wenaFallbackReason = 'flag_off';
  } else {
    // RAP enabled, quest mode active, but NOT English — keep Socratic quest prompt.
    wenaFallbackReason = 'quest_mode_override';
  }
  ...
  try {
    ...
    if (hasImage) {
      ...
      // Text-only path: build proper multi-turn messages array for full conversation context.
      const openAiMessages = [
        { role: 'system', content: systemPrompt },
        ...rawMessages.map(m => ({ role: m.role === 'assistant' ? 'assistant' : 'user', content: m.content || '' })),
      ];
      reply = await callAI('chat', { messagesArray: openAiMessages, temperature: 0.4, maxTokens: 1024 });
    }
    ...
    if (wenaPromptVersion) responseBody.wena_prompt_version = wenaPromptVersion;
    // Sprint 4: echo the resolved mode back so the client can include it as
    // `lastWenaMode` on its NEXT request — needed for CHECK_RESPONSE detection
    // across turns. 'LEGACY' when RAP did not fire for any reason.
    responseBody.wena_mode = wenaRapFired ? wenaMode : 'LEGACY';

    // ── Sprint 4: per-turn telemetry (fire-and-forget) ───────────────────
    // Compute DIRECT_TEACH faithfulness only when applicable. Other modes
    // log null for the metric so the dashboard's overlap histogram only
    // shows DIRECT_TEACH events.
    let teachOverlap = null;
    if (wenaRapFired && wenaMode === 'DIRECT_TEACH' && wenaCell?.foundational_teach_script && reply) {
      teachOverlap = computeKeywordOverlap(wenaCell.foundational_teach_script, reply);
    }
    // Intentionally NOT awaited — chat must return to the student without
    // blocking on the telemetry insert. logPedagogyEvent never throws.
    logPedagogyEvent({
      studentId:      rapStudentId || null,
      parentId:       rapParentId  || null,
      conversationId: rapConversationId || null,
      turnIndex:      rawMessages.length - 1,
      levelRaw:       rapRawLevel || null,
      subject:        rapSubject  || null,
      topic:          rapTopicIn  || null,
      subTopic:       rapSubTopicIn || null,
      rapEnabled:     RAP_ENABLED,
      promptVersion:  wenaRapFired ? WENA_PROMPT_VERSION : null,
      mode:           wenaRapFired ? wenaMode : 'LEGACY',
      previousMode:   rapLastWenaMode || null,
      cellHitKind:    wenaRapFired ? wenaCellHitKind : 'n/a',
      stallCount:     consecutiveStalls(rawMessages),
      hasImage:       !!hasImage,
      teachScriptKeywordOverlap: teachOverlap,
      outputLengthChars: reply.length,
      metadata: {
        fallback_reason: wenaRapFired ? null : wenaFallbackReason,
        canary_pct: process.env.WENA_RAP_ROLLOUT_PCT || null,
        env_flag:   process.env.WENA_RAP_ENABLED || null,
        // Sprint 8b: true when RAP fired AND a quest session was active.
        // Reads quest_override=true distinguishes Day 2 Socratic-overrides
        // from regular non-quest English RAP sessions in the dashboard.
        quest_override: wenaQuestOverride
      }
    });
...
export async function handleSummarizeChat(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  // Stage label kept around so the catch block can return a code that pinpoints
  // which step failed without leaking error internals to the client.
  let stage = 'init';
  let rawResponse = '';
  try {
    const user = await getUserFromToken(req.headers.authorization);
    ...
    stage = 'ai_call';
    rawResponse = await callAI('summarize', {
    ...
    let generatedData;
    try {
      generatedData = JSON.parse(stripMarkdownFences(rawResponse));
    } catch (parseErr) {
      console.error('[summarize-chat] JSON parse failed. Raw (first 400 chars):', String(rawResponse).slice(0, 400));
      throw new Error(`AI response was not valid JSON: ${parseErr.message}`);
    }
    if (!generatedData.title || !generatedData.content_html) {
      console.error('[summarize-chat] Missing schema fields. Got keys:', Object.keys(generatedData || {}));
      throw new Error('AI response missing title or content_html.');
    }
    ...
    if (dbErr) {
      console.error('[summarize-chat] DB insert failed:', dbErr.message, dbErr.details || '');
      throw new Error(`study_notes insert failed: ${dbErr.message}`);
    }
    return res.status(200).json({ success: true });
  } catch (err) {
    console.error(`[summarize-chat] stage=${stage} error:`, err.message || err);
    return res.status(500).json({
      error: 'Failed to generate and save the study note.',
      stage,
      detail: (err && err.message) ? String(err.message).slice(0, 240) : 'unknown',
    });
  }
}
  ...
  try {
    switch (event.type) {
      ...
      case 'customer.subscription.updated': {
        ...
        // Activity log: plan_changed when Stripe reports the price line items
        // changed in this update. previous_attributes.items is present only on
        // an actual line-item swap.
        const itemsChanged = !!event.data.previous_attributes?.items;
        if (itemsChanged && plan) {
          const uidForActivity = sub.metadata?.supabase_user_id;
          if (uidForActivity) {
            await logActivity({
              adminDb: db,
              profileId: uidForActivity,
              eventType: 'plan_changed',
              metadata: { plan, billing_cycle: cycle || null, stripe_subscription_id: sub.id },
            });
          }
        }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (uid) {
          await db.from('profiles').update({ subscription_tier: 'trial', max_children: 1 }).eq('id', uid);
          await logActivity({
            adminDb: db,
            profileId: uid,
...
2. Cognitive Skill: 'cognitive_skill' MUST be EXACTLY one of these 9 canonical values (master_question_template/_base.md AO map, router v6.0): 'Factual Recall', 'Conceptual Understanding', 'Literal Retrieval', 'Routine Application', 'Inferential Reasoning', 'Contextual Clues', 'Non-Routine / Heuristics', 'Synthesis & Evaluation', or 'CER (Claim-Evidence-Reasoning)'. AO1 = first three, AO2 = next three, AO3 = last three. No other strings allowed.
...
export async function handleGenerate(req, res) {
  ...
  const provider = `${AI_ROUTING.bulk_question.provider}-${AI_ROUTING.bulk_question.model}`;
  try {
    const rawResponse = await callAI('bulk_question', {
      systemPrompt: GENERATE_PROMPTS[questionType],
      userPrompt:   buildGenerateUserPrompt(questionType, questionCount, subject, level, topic, difficulty || 'Standard'),
      maxTokens:    8192,
    });
    const questions = parseJsonArray(rawResponse.trim());
    const adminDb = getAdminClient();
    const { accepted, rejected } = await filterByCanon(adminDb, questions);
    if (rejected.length > 0) {
      console.warn(`[generate] Rejected ${rejected.length}/${questions.length} off-canon questions:`, rejected.slice(0, 3));
    }
    return res.status(200).json({ questions: accepted, rejected, ai_provider: provider });
  } catch (err) {
...
function buildGQSchemaInstructions(subject, level, topic, type, difficulty, count) {
  // Inline the canonical sub_topic list for this (subject, topic) so the
  // model picks one of the locked strings. The DB-level canon validator
  // (filterByCanon) will reject anything off-list, so prompting for canon
  // up-front reduces wasted generations + rejections.
  const canonSubs = getSubTopics(subject.toLowerCase(), topic);
  const subTopicGuidance = canonSubs.length > 0
    ? `\nThe sub_topic field MUST be one of (verbatim, exact spelling): ${JSON.stringify(canonSubs)}. Pick whichever fits the question best.\n`
    : `\nThe sub_topic field should reflect the specific MOE concept tested (Title-Cased).\n`;
  const base = `\nGenerate exactly ${count} ${subject} question(s) for ${level} students on "${topic}".\nDifficulty: ${difficulty}. Use Singapore names, food, places. British English spelling.\nReturn ONLY a valid JSON array.\n\nRequired fields: id, subject, level, topic, sub_topic, difficulty, type, marks (1-4), question_text, worked_solution (min 3 steps).${subTopicGuidance}`;
  const schemas = {
...
export async function handleGenerateQuestion(req, res) {
  ...
  try {
    ...
    const schemaValid = parseJsonArray(responseText).filter(q => { try { return validateGQQuestion(q); } catch { return false; } }).map(q => ({ ...q, source: 'ai', aiProvider: provider }));
    if (schemaValid.length === 0) return res.status(500).json({ error: 'AI returned no valid questions' });
    const adminDb = getAdminClient();
    const { accepted, rejected } = await filterByCanon(adminDb, schemaValid);
    if (rejected.length > 0) {
      console.warn(`[generate-question] Rejected ${rejected.length}/${schemaValid.length} off-canon questions:`, rejected.slice(0, 3));
    }
    if (accepted.length === 0) {
      return res.status(422).json({ error: 'All AI-generated questions violated the canonical taxonomy', rejected, provider });
    }
    return res.status(200).json({ questions: accepted, rejected, provider });
  } catch (err) {
...
export async function handleGenerateQuest(req, res) {
  ...
  try {
    ...
    if (!topic.trim() || topic.trim().toLowerCase() === 'mixed' || topic.trim().toLowerCase() === 'all') {
      return res.status(400).json({ error: 'A specific topic is required to generate a quest. Mixed-topic quizzes cannot trigger a quest.' });
    }
    const adminDb = getAdminClient();
...
// ─── Cognitive Diagnosis (Pillar 2 — Analyse Weakness) ───────────────────────
//
// Powers the parent-facing hero sentence on progress.html. Reads BKT mastery
// with sub_topic granularity, finds the weakest (topic, sub_topic) pair the
// student has practised enough to trust (≥3 topic attempts, ≥2 sub_topic
// attempts), counts how many downstream topics depend on it, and composes
// a one-line plain-English diagnosis.
//
// Honest Compass guardrail: every template describes the CURRENT STATE.
// No "improving", "great work", "well done", "amazing", "keep up". If
// `weakest_topic` can't be derived, we never invent one — we use the
// early-stage / empty-state template instead.

const _SUBJECT_DISPLAY = {
  mathematics: 'Mathematics',
  science:     'Science',
  english:     'English',
};

function _probabilityToALBand(p) {
  // Same thresholds as progress.js + handlers.js handleFamilyActivity helper.
...
function _countDirectDependents(subjectKey, topic) {
  const subjectMap = SYLLABUS_DEPENDENCIES[subjectKey] || {};
  let count = 0;
  for (const [otherTopic, node] of Object.entries(subjectMap)) {
    if (otherTopic === topic) continue;
    if ((node.prerequisites || []).includes(topic)) count++;
  }
  return count;
}

export async function runCognitiveDiagnosis(studentId, subject) {
  const subjectKey     = String(subject || '').toLowerCase();
  const subjectDisplay = _SUBJECT_DISPLAY[subjectKey] || subject || 'this subject';

  const adminDb = getAdminClient();

  // Student name — drives the sentence's leading clause. Fall back gracefully
  // if the row is missing rather than failing the whole diagnosis.
  let name = 'Your child';
  try {
    const { data: stu } = await adminDb.from('students').select('name').eq('id', studentId).maybeSingle();
    if (stu?.name) name = stu.name;
  } catch { /* tolerate */ }

  const empty = {
    al_band: null,
    weakest_topic: null,
    weakest_sub_topic: null,
    dependent_topic_count: 0,
    hero_sentence: `${name} hasn't attempted ${subjectDisplay} yet — start with one quiz to begin diagnosis.`,
  };

  let mastery;
  try {
    const { data, error } = await adminDb.from('mastery_levels')
    ...
    console.error('[runCognitiveDiagnosis] mastery_levels read failed:', err.message || err);
    return empty;
  }

  if (mastery.length === 0) return empty;

  // Group by topic; weight probability by attempts so a low-attempt sub_topic
  // doesn't dominate. Filter topics with <3 attempts so we never flag a topic
  // the student barely touched.
  const topicMap = {};
  for (const row of mastery) {
    if (!topicMap[row.topic]) topicMap[row.topic] = { totalAttempts: 0, weightedProb: 0, subRows: [] };
    const a = row.attempts || 0;
    topicMap[row.topic].totalAttempts += a;
    topicMap[row.topic].weightedProb  += (row.probability || 0) * a;
    topicMap[row.topic].subRows.push(row);
  }

  let weakestTopic = null;
  let lowestMean   = 1.0;
  for (const [topic, data] of Object.entries(topicMap)) {
    if (data.totalAttempts < 3) continue;
    const mean = data.weightedProb / data.totalAttempts;
    if (mean < lowestMean) { lowestMean = mean; weakestTopic = topic; }
  }

  if (!weakestTopic) {
    // Has mastery rows but none qualify — early-stage student. Compute an
    // overall AL band so the sentence still grounds the parent in something
    // factual rather than dropping to the empty-state template.
    const totalA = mastery.reduce((s, r) => s + (r.attempts || 0), 0);
    const wSum   = mastery.reduce((s, r) => s + (r.probability || 0) * (r.attempts || 0), 0);
    const overall = totalA > 0 ? wSum / totalA : null;
    const al_band = overall !== null ? _probabilityToALBand(overall) : null;
    const lead = al_band ? `${name} is at ${al_band} in ${subjectDisplay}` : `${name} is in the early stages of ${subjectDisplay}`;
    return {
      al_band,
      weakest_topic: null,
      weakest_sub_topic: null,
      dependent_topic_count: 0,
      hero_sentence: `${lead} — keep practising to build a clearer picture.`,
    };
  }

  // Weakest sub_topic inside the weakest topic — only consider rows with
  // attempts >= 2 and a non-null sub_topic. Avoids flagging a sub_topic the
  // student tried once.
  let weakestSubTopic = null;
  let lowestSubProb   = 1.1;
  for (const r of topicMap[weakestTopic].subRows) {
    if (!r.sub_topic) continue;
    if ((r.attempts || 0) < 2) continue;
    if ((r.probability ?? 1) < lowestSubProb) {
      lowestSubProb   = r.probability;
      weakestSubTopic = r.sub_topic;
    }
  }

  const al_band               = _probabilityToALBand(lowestMean);
  const dependent_topic_count = _countDirectDependents(subjectKey, weakestTopic);

  const verb            = dependent_topic_count > 0 ? 'focus on' : 'strengthen';
  const dependentsTail  = dependent_topic_count > 0
    ? `to unlock ${dependent_topic_count} dependent topic${dependent_topic_count !== 1 ? 's' : ''}`
    : 'to lift overall mastery';
  const subTopicClause  = weakestSubTopic ? ` (${weakestSubTopic})` : '';
  const lead            = al_band
    ? `${name} is at ${al_band} in ${subjectDisplay}`
    : `${name} is in the early stages of ${subjectDisplay}`;

  return {
    al_band,
    weakest_topic:         weakestTopic,
    weakest_sub_topic:     weakestSubTopic,
    dependent_topic_count,
    hero_sentence: `${lead} — ${verb} ${weakestTopic}${subTopicClause} ${dependentsTail}.`,
  };
}
...
export async function handleAnalyzeWeakness(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  try {
    // Accept both shapes — legacy progress.js toggleDeepDive sends snake_case
    // (student_id), Week 2 hero diagnosis path sends camelCase (studentId).
    const body      = req.body || {};
    const studentId = body.studentId || body.student_id;
    const subject   = body.subject;
    if (!studentId || !subject) return res.status(400).json({ error: 'Missing studentId or subject' });
    const db = getAdminClient();
    // Pillar 2 hero sentence — runs in parallel with the legacy weakness
    // analysis so the existing fields keep working while new clients can
    // read response.diagnosis.hero_sentence directly.
    const diagnosisPromise = runCognitiveDiagnosis(studentId, subject);
    ...
    const prereqs = getPrerequisites(subject, weakestTopic);
...
// ─── HANDLER: /api/syllabus-tree ─────────────────────────────────────────────
//
// Public endpoint exposing SYLLABUS_DEPENDENCIES so the frontend dependency-tree
// component can hydrate without a mirror file (no drift surface). The map is
// product knowledge, not user data — safe to cache aggressively.
...
// ─── HANDLER: /api/recent-attempts ───────────────────────────────────────────
//
// Powers the heatmap modal on progress.html. Returns the last N question
// attempts for a (student, topic, sub_topic) tuple plus aggregate stats over
// the last 50 rows. Auth via Bearer token; ownership enforced by parent_id.
// sub_topic param empty string OR omitted → matches NULL sub_topic rows so
// legacy attempts (pre-canon) still surface.
...
export async function handleRecentAttempts(req, res) {
  ...
  try {
    ...
    const subTopic  = params.get('sub_topic'); // null = not provided, '' = explicit null match
    const limit     = Math.min(Math.max(1, parseInt(params.get('limit') || '5', 10) || 5), 20);
    ...
    // Ownership check — service role bypasses RLS, so verify parent_id manually.
    ...
    // Build attempts query. Empty/missing sub_topic → match NULL.
    const matchesNullSub = subTopic === null || subTopic === '';

    let attemptsQ = adminDb
      ...
      .limit(limit);
    if (matchesNullSub) attemptsQ = attemptsQ.is('sub_topic', null);
    else                attemptsQ = attemptsQ.eq('sub_topic', subTopic);

    const { data: attemptsRows, error: attErr } = await attemptsQ;
    if (attErr) throw attErr;

    // Stats: accuracy is computed from the last 50 attempts (representative
    // recent-window — full lifetime would scale poorly for power users), but
    // total_attempts is the TRUE lifetime count via { count: 'exact' } so the
    // modal header doesn't lie ("50 attempts" when the kid actually has 200).
    let statsQ = adminDb
      .from('question_attempts')
      ...
      .order('created_at', { ascending: false })
      .limit(50);
    if (matchesNullSub) statsQ = statsQ.is('sub_topic', null);
    else                statsQ = statsQ.eq('sub_topic', subTopic);

    const { data: statsRows, count: totalAttempts, error: statsErr } = await statsQ;
    if (statsErr) throw statsErr;

    const sampleCount  = (statsRows || []).length;
    const correctCount = (statsRows || []).filter(r => r.correct).length;
    const accuracy     = sampleCount > 0 ? correctCount / sampleCount : null;

    // Mastery probability for the same cell — pulled from the BKT row.
    let masteryQ = adminDb
      .from('mastery_levels')
      .select('probability, attempts')
      .eq('student_id', studentId)
      .eq('topic', topic);
    if (matchesNullSub) masteryQ = masteryQ.is('sub_topic', null);
    else                masteryQ = masteryQ.eq('sub_topic', subTopic);
    const { data: masteryRow } = await masteryQ.maybeSingle();

    return res.status(200).json({
      topic,
      sub_topic: matchesNullSub ? null : subTopic,
    ...
    console.error('[recent-attempts] Error:', err.message || err);
  ...
  try {
    ...
    if (action === 'schedule') {
      ...
      await logActivity({
        ...
        metadata: { resume_at: resumeDate.toISOString(), pause_activates_at: periodEnd?.toISOString() || null },
        req,
      });
...
// Append-only ledger surfaced to parents on /account.html (Phase 2 of the
// account advanced-features plan). Service-role inserts only — clients can
// read their own rows via RLS but cannot fabricate events. See migration
// 024_account_activity.sql for schema + policy.
...
// Static salt — makes the hash irreversible without the source. Stable across
// deploys so two events from the same IP collapse to the same ip_hash, which
// is the property we want for "logged in from a new device" reasoning.
const _IP_HASH_SALT = 'shl-account-activity-v1';
...
function _hashIp(ip) {
  if (!ip) return null;
  return crypto.createHash('sha256').update(_IP_HASH_SALT + ':' + ip).digest('hex').slice(0, 16);
}

// Server-side logger — non-throwing on purpose. Activity logging must never
// block the action it observes. Validates event_type against the allow list.
async function logActivity({ adminDb, profileId, eventType, metadata, req }) {
  if (!profileId) return;
  if (!ACCOUNT_ACTIVITY_TYPES.has(eventType)) {
    console.warn('[activity] rejected unknown event_type:', eventType);
    return;
  }
  try {
    const db = adminDb || getAdminClient();
    const ip      = req ? _extractIp(req)                   : null;
    const ipHash  = _hashIp(ip);
    const ua      = req?.headers?.['user-agent'] || null;
    const safeUa  = typeof ua === 'string' ? ua.slice(0, 500) : null;
    ...
    console.error('[activity] insert failed:', err?.message || err);
  }
}

// ─── HANDLER: /api/account-activity ──────────────────────────────────────────
// GET — returns the most recent activity rows for the authenticated parent.
//   ?limit=N   default 8, capped at 100

export async function handleAccountActivity(req, res) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const user = await getUserFromToken(req.headers.authorization);
    if (!user) return res.status(401).json({ error: 'Unauthorised' });
    const params  = new URLSearchParams((req.url || '').split('?')[1] || '');
    const limit   = Math.min(100, Math.max(1, parseInt(params.get('limit') || '8', 10) || 8));
    const adminDb = getAdminClient();
    const { data, error } = await adminDb.from('account_activity')
    ...
    console.error('[account-activity] Error:', err.message || err);
    return res.status(500).json({ error: 'Could not load account activity.' });
  }
}

// ─── HANDLER: /api/log-activity ───────────────────────────────────────────────
// POST — client-side flows (login, MFA, password / email change) call this so
// the server controls the IP hash + UA capture and validates event_type.
// Body: { event_type, metadata }

export async function handleLogActivity(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const user = await getUserFromToken(req.headers.authorization);
    if (!user) return res.status(401).json({ error: 'Unauthorised' });
    const { event_type, metadata } = req.body || {};
    if (!event_type || !ACCOUNT_ACTIVITY_TYPES.has(event_type)) {
...
// GET — small "this week" summary card on the dashboard (Phase 3 of the
// dashboard advanced-features plan). Three stat blocks + a one-sentence
// last-week pacer. Server-side compute so the client doesn't have to
// re-implement the SGT-week boundary math.
//
// Response: { this_week:  { questions, mastery_gains, streak_days, range_label },
//             last_week:  { questions, mastery_gains } }
//
// Cached for 5 minutes per parent in-process. Vercel functions are short-
// lived so cross-invocation hits are rare, but within a single dashboard
// session multiple loads / refreshes share the warm cache.

const _digestCache = new Map();
const _DIGEST_TTL_MS = 5 * 60 * 1000;

function _digestCacheGet(profileId) {
  const entry = _digestCache.get(profileId);
  if (!entry) return null;
  if (Date.now() - entry.at > _DIGEST_TTL_MS) {
    _digestCache.delete(profileId);
    return null;
  }
  return entry.data;
}
function _digestCacheSet(profileId, data) {
  _digestCache.set(profileId, { at: Date.now(), data });
}

// SGT week boundaries — Monday 00:00 SGT is the canonical reset per the
// handoff. Returns ISO timestamps suitable for Postgres .gte/.lt comparisons
// against created_at columns, plus a date-string range for daily_usage.date
// (which is a plain DATE column).
function _weekRangesSgt() {
  ...
  const wkIdx   = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].indexOf(get('weekday'));
  // Days since Monday (Mon=0, Tue=1, ... Sun=6)
  const daysSinceMon = (wkIdx + 6) % 7;
  ...
  const thisWeekStartDate = shiftDate(ymd, -daysSinceMon);
  const lastWeekStartDate = shiftDate(thisWeekStartDate, -7);

  // Monday 00:00 in SGT == Sunday 16:00 in UTC. The +08:00 suffix lets new
  // Date(...) parse the local-Monday-midnight into the right UTC instant.
  const thisWeekStart = new Date(`${thisWeekStartDate}T00:00:00+08:00`).toISOString();
  const lastWeekStart = new Date(`${lastWeekStartDate}T00:00:00+08:00`).toISOString();
  const nowIso        = new Date().toISOString();

  // Range label for the card title: "Mon 22 Apr — today"
  const labelDate = new Date(`${thisWeekStartDate}T00:00:00+08:00`);
  const startStr  = labelDate.toLocaleDateString('en-SG', {
  ...
  return {
    ...
    thisWeekStartDateStr:  thisWeekStartDate,
    lastWeekStartDateStr:  lastWeekStartDate,
    rangeLabel:            `${startStr} — today`,
  };
}

export async function handleWeeklyDigest(req, res) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const user = await getUserFromToken(req.headers.authorization);
    if (!user) return res.status(401).json({ error: 'Unauthorised' });

    const cached = _digestCacheGet(user.id);
    if (cached) return res.status(200).json(cached);

    const adminDb = getAdminClient();

    const { data: students, error: stuErr } = await adminDb.from('students')
      .select('id')
      .eq('parent_id', user.id);
    if (stuErr) throw stuErr;
    const sids = (students || []).map(s => s.id);

    const range = _weekRangesSgt();
    const empty = {
      this_week: { questions: 0, mastery_gains: 0, streak_days: 0, range_label: range.rangeLabel },
      last_week: { questions: 0, mastery_gains: 0 },
    };
    if (sids.length === 0) {
      _digestCacheSet(user.id, empty);
      return res.status(200).json(empty);
    }

    // Run the 5 counts in parallel:
    //   thisWeek questions, thisWeek mastery_gains, thisWeek streak_days,
    //   lastWeek questions, lastWeek mastery_gains.
    // For mastery_gains we use xp_events.event_type='mastery_gain' — the
    // canonical signal once the daily mastery cron lands. mastery_levels_
    // snapshots will eventually let us compute actual band-jumps, but the
    // xp_event is the same row that snapshot cron will emit, so this is
    // forward-compatible.

    const xpFilter = sids.length > 0 ? sids : ['00000000-0000-0000-0000-000000000000'];

    async function countQuestions(fromIso, toIso) {
      const { count } = await adminDb.from('question_attempts')
    ...
    async function countMasteryGains(fromIso, toIso) {
      const { count } = await adminDb.from('xp_events')
        .select('id', { count: 'exact', head: true })
        .in('student_id', xpFilter)
        .eq('event_type', 'mastery_gain')
        .gte('created_at', fromIso)
        .lt('created_at', toIso);
      return count || 0;
    }
    async function streakDaysThisWeek(fromDateStr, toDateStr) {
      // daily_usage.date is a DATE column. Pull rows in the range and count
      // distinct dates client-side (Supabase JS doesn't do COUNT DISTINCT
      // directly).
      const { data } = await adminDb.from('daily_usage')
        .select('date')
        .in('student_id', sids)
        .gte('date', fromDateStr)
        .lte('date', toDateStr);
      const set = new Set();
      (data || []).forEach(r => { if (r.date) set.add(r.date); });
      return set.size;
    }

    const [
      qThis, mThis, sThis, qLast, mLast,
    ] = await Promise.all([
      countQuestions(range.thisWeekStart, range.thisWeekEnd),
      countMasteryGains(range.thisWeekStart, range.thisWeekEnd),
      streakDaysThisWeek(range.thisWeekStartDateStr, _todayInSgt()),
      countQuestions(range.lastWeekStart, range.lastWeekEnd),
      countMasteryGains(range.lastWeekStart, range.lastWeekEnd),
    ]);
    ...
    _digestCacheSet(user.id, payload);
    return res.status(200).json(payload);
  } catch (err) {
    console.error('[weekly-digest] Error:', err.message || err);
    return res.status(500).json({ error: 'Could not load weekly digest.' });
  }
}

function _todayInSgt() {
  const fmt = new Intl.DateTimeFormat('en-CA', {
    timeZone: 'Asia/Singapore',
    year: 'numeric', month: '2-digit', day: '2-digit',
  });
...
// ─── HANDLER: /api/family-activity ────────────────────────────────────────────
// GET — most recent learning events across all of this parent's learners.
// Used by the dashboard "What's been happening" feed (Phase 2 of the
// dashboard advanced-features plan).
//
// Pulls in parallel from 5 tables, normalises into one event shape, sorts
// desc by occurred_at, returns the top N.
//
// Query: ?limit=N (default 5, capped 20)
// Response: { events: [{type, student_id, student_name, summary, occurred_at, action_url}] }

export async function handleFamilyActivity(req, res) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const user = await getUserFromToken(req.headers.authorization);
    if (!user) return res.status(401).json({ error: 'Unauthorised' });
    ...
    // Get this parent's students (also gives us names for the summaries)
    const { data: students, error: stuErr } = await adminDb.from('students')
      .select('id, name')
      .eq('parent_id', user.id);
    if (stuErr) throw stuErr;
    if (!students || students.length === 0) return res.status(200).json({ events: [] });

    const sids   = students.map(s => s.id);
    const nameOf = id => (students.find(x => x.id === id) || {}).name || 'Your child';

    // Per-source headroom — fetch limit + 5 from each table so the post-merge
    // sort still has enough recent rows to fill `limit` from any single source.
    const cap = limit + 5;

    const [qa, ml, rq, rqDone, sn, er] = await Promise.all([
      // quiz_attempts: student-completed quizzes (no in_progress state today)
      adminDb.from('quiz_attempts')
        .select('id, student_id, subject, topic, score, total_questions, completed_at')
        .in('student_id', sids)
        .order('completed_at', { ascending: false })
        .limit(cap),
      // mastery_levels: only "notable" levels — probability >= 0.70 (≈ AL3 or
      // better). Mirrors the dashboard's existing AL band thresholds so the
      // feed surfaces real progress rather than every minor probability bump.
      adminDb.from('mastery_levels')
        .select('id, student_id, subject, topic, probability, last_updated')
        .in('student_id', sids)
        .gte('probability', 0.70)
        .order('last_updated', { ascending: false })
        .limit(cap),
      // remedial_quests created — Plan Quest issued
      adminDb.from('remedial_quests')
        .select('id, student_id, topic, created_at')
        .in('student_id', sids)
        .order('created_at', { ascending: false })
        .limit(cap),
      // remedial_quests completed — Plan Quest finished (status='completed').
      // Surfaces the "Lily completed English Synthesis Quest" line in the
      // dashboard activity card.
      adminDb.from('remedial_quests')
        .select('id, student_id, subject, topic, day3_outcome, updated_at')
        .in('student_id', sids)
        .eq('status', 'completed')
        .order('updated_at', { ascending: false })
        .limit(cap),
      // study_notes: each saved Miss Wena summary
      adminDb.from('study_notes')
        .select('id, student_id, subject, topic, title, created_at')
        .in('student_id', sids)
        .order('created_at', { ascending: false })
        .limit(cap),
      // exam_results: WA / EOY / PSLE-style paper completions
      adminDb.from('exam_results')
        .select('id, student_id, subject, level, exam_type, score, total_marks, completed_at')
        .in('student_id', sids)
        .order('completed_at', { ascending: false })
        .limit(cap),
    ]);

    function probabilityToBandNumber(p) {
      // Same thresholds as public/pages/dashboard.html probabilityToALBand
      if (p >= 0.90) return 1;
      if (p >= 0.80) return 2;
      if (p >= 0.70) return 3;
      if (p >= 0.60) return 4;
    ...
    (qa.data || []).forEach(r => {
      ...
      events.push({
        ...
        summary:      `${nameOf(r.student_id)} completed ${r.topic} quiz · ${score}/${total}`,
        occurred_at:  r.completed_at,
        action_url:   `/pages/progress.html?student=${encodeURIComponent(r.student_id)}`,
      });
    });

    (ml.data || []).forEach(r => {
      const band = probabilityToBandNumber(r.probability || 0);
      events.push({
        type: 'mastery_gained',
        student_id:   r.student_id,
        student_name: nameOf(r.student_id),
        summary:      `${nameOf(r.student_id)} reached AL${band} on ${r.topic}`,
        occurred_at:  r.last_updated,
        action_url:   `/pages/progress.html?student=${encodeURIComponent(r.student_id)}`,
      });
    });
    ...
    (rq.data || []).forEach(r => {
      events.push({
        ...
        student_id:   r.student_id,
        student_name: nameOf(r.student_id),
        summary:      `Plan Quest available for ${nameOf(r.student_id)} on ${r.topic}`,
        occurred_at:  r.created_at,
        action_url:   `/quest?id=${encodeURIComponent(r.id)}`,
      });
    });

    // Capitalises the lowercase subject value stored on remedial_quests
    // (mathematics → Mathematics) so the activity line reads naturally.
    const subjectLabel = s => s ? s.charAt(0).toUpperCase() + s.slice(1) : '';

    (rqDone.data || []).forEach(r => {
      const subj = subjectLabel(r.subject);
      // "<Name> completed English Synthesis Quest" — matches the copy spec.
      const summary = `${nameOf(r.student_id)} completed ${subj} ${r.topic} Quest`;
      events.push({
    ...
    (sn.data || []).forEach(r => {
      const cleanTitle = (r.title || 'a study note').toString().slice(0, 80);
      events.push({
        type: 'study_note',
        student_id:   r.student_id,
        student_name: nameOf(r.student_id),
        summary:      `${nameOf(r.student_id)} saved a Miss Wena note: ${cleanTitle}`,
        occurred_at:  r.created_at,
        action_url:   `/pages/progress.html?student=${encodeURIComponent(r.student_id)}`,
      });
    });
    ...
    (er.data || []).forEach(r => {
      ...
      events.push({
        ...
        student_id:   r.student_id,
        student_name: nameOf(r.student_id),
        summary:      `${nameOf(r.student_id)} completed ${r.level} ${r.subject} ${r.exam_type} · ${score}/${total}`,
        occurred_at:  r.completed_at,
        action_url:   `/pages/progress.html?student=${encodeURIComponent(r.student_id)}`,
      });
    });

    // Filter out any rows missing occurred_at (defensive — should be NOT NULL),
    // then desc-sort + slice.
    const filtered = events.filter(e => !!e.occurred_at);
    filtered.sort((a, b) => new Date(b.occurred_at).getTime() - new Date(a.occurred_at).getTime());
    return res.status(200).json({ events: filtered.slice(0, limit) });
  } catch (err) {
    console.error('[family-activity] Error:', err.message || err);
    return res.status(500).json({ error: 'Could not load family activity.' });
  }
}

// ─── HANDLER: /api/learner-export ─────────────────────────────────────────────
// GET — exports a single learner's full quiz/quest/exam history as JSON.
// Used when a parent wants to share one child's data with a tutor without
// exposing the rest of the family. Distinct from /api/export which dumps the
// whole account.
//
// Query: ?student_id=<uuid>
// Auth:  Bearer token; parent must own the student row.
// Response: application/json, Content-Disposition attachment.

export async function handleLearnerExport(req, res) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const user = await getUserFromToken(req.headers.authorization);
    if (!user) return res.status(401).json({ error: 'Unauthorised' });
    ...
    // Ownership check — RLS already enforces this for client paths, but the
    // export goes via service role so we belt-and-suspenders verify here.
    const { data: student, error: studentErr } = await adminDb.from('students')
      .select('id, name, level, parent_id, selected_subject, photo_url, created_at')
      .eq('id', studentId)
      .maybeSingle();
    if (studentErr) throw studentErr;
    if (!student) return res.status(404).json({ error: 'Learner not found.' });
    if (student.parent_id !== user.id) return res.status(403).json({ error: 'Forbidden — that learner does not belong to this account.' });

    // Parallel pull across all child-data tables
    const [qa, qua, ml, sn, rq, er] = await Promise.all([
      adminDb.from('quiz_attempts')    .select('*').eq('student_id', studentId),
      adminDb.from('question_attempts').select('*').eq('student_id', studentId),
      adminDb.from('mastery_levels')   .select('*').eq('student_id', studentId),
      adminDb.from('study_notes')      .select('*').eq('student_id', studentId),
      adminDb.from('remedial_quests')  .select('*').eq('student_id', studentId),
      adminDb.from('exam_results')     .select('*').eq('student_id', studentId),
    ]);
    ...
    const payload = {
      ...
      question_attempts: qua.data || [],
      mastery_levels:    ml.data  || [],
      study_notes:       sn.data  || [],
      remedial_quests:   rq.data  || [],
      exam_results:      er.data  || [],
    };

    // Log the export — non-blocking, must not fail the download
    logActivity({
      adminDb,
      profileId: user.id,
      eventType: 'data_exported',
      metadata: { scope: 'learner', student_id: student.id, learner_name: student.name },
      req,
    }).catch(function () { /* swallow */ });

    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="learner-data-' + student.id + '.json"');
    return res.status(200).send(JSON.stringify(payload, null, 2));
  } catch (err) {
    console.error('[learner-export] Error:', err.message || err);
    return res.status(500).json({ error: 'Could not export learner data.' });
  }
}
...
export async function handleAccountDelete(req, res) {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Account deletion failed. Please contact support at support@superholiclab.com.' });
  }
}
...
// ─── HANDLER: /api/contact ────────────────────────────────────────────────────
// Public contact form submission. Writes to contact_submissions (Supabase),
// fires an auto-acknowledgement to the parent, and a triage alert to the
// internal inbox. Honeypot trips return 200 OK silently (so the bot leaves us
// alone) but flag the row as is_honeypot=true so we can audit volume later.
//
// Topic routing maps each topic to a logical queue (routed_to). Use this in
// the admin dashboard "At-Risk Radar" to triage faster.
//
// IMPORTANT: This is a public endpoint. Rate-limited at 5 req / 10 min per IP
// via api/index.js gateway middleware — do not invoke heavy work here.

const CONTACT_TOPIC_ROUTING = {
  'General Inquiry':              'support',
  'Technical Support':            'tech',
  'Billing & Subscription':       'billing',
  'Refund Request':               'billing',
  'Feedback / Feature Request':   'support',
...
export async function handleContact(req, res) {
  ...
  try {
    ...
    // ─── Honeypot trip ── silently "succeed" so bot scripts don't retry.
    // We still log to Supabase with is_honeypot=true for visibility.
    if (company && company.trim()) {
      const adminDb = getAdminClient();
      await adminDb.from('contact_submissions').insert({
        name: (name || '').slice(0, 200),
        email: (email || '').toLowerCase().slice(0, 200),
        topic: (topic || 'unknown').slice(0, 100),
      ...
      console.log(`[contact] Honeypot trip from ${email || 'anon'} — silently dropped`);
      return res.status(200).json({ success: true });
    }
    ...
    const cleanTopic   = (topic || 'General Inquiry').trim().slice(0, 100);
    const cleanMessage = message.trim().slice(0, 5000);
    const routedTo     = CONTACT_TOPIC_ROUTING[cleanTopic] || 'support';
    ...
    console.log(`[contact] New submission #${row.id} from ${cleanEmail}, topic: ${cleanTopic}, routed: ${routedTo}`);

    // ─── Fire-and-forget emails (do not await — keeps the response snappy)
    // Both calls fail-silent. The row is already saved; emails are extras.
    Promise.allSettled([
      sendAutoAcknowledgement({ name: cleanName, email: cleanEmail, topic: cleanTopic }),
      sendInternalAlert({
        id: row.id,
        name: cleanName,
        email: cleanEmail,
      ...
      results.forEach((r, i) => {
        if (r.status === 'rejected') {
          console.error(`[contact] Email ${i === 0 ? 'auto-ack' : 'internal-alert'} failed:`, r.reason);
        }
      });
    });
    ...
    return res.status(500).json({
      error: 'Failed to send message. Please email us directly at support@superholiclab.com.'
    });
  }
}

// ─── Auto-acknowledgement to the parent who submitted the form
async function sendAutoAcknowledgement({ name, email, topic }) {
  const html = `
    <div style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; color: #2C3E3A; line-height: 1.6;">
      <p style="font-family: 'Bebas Neue', sans-serif; letter-spacing: 0.04em; color: #B76E79; font-size: 13px; margin: 0 0 16px 0;">SUPERHOLIC LAB · SUPPORT</p>
      <h1 style="font-family: 'Bebas Neue', sans-serif; font-size: 32px; color: #2C3E3A; letter-spacing: 0.02em; margin: 0 0 16px 0;">We've got your message.</h1>
      <p>Hi ${escapeHtml(name)},</p>
      <p>Thanks for reaching out. We received your enquiry about <strong>${escapeHtml(topic)}</strong> and will reply by email within <strong>24 hours, Mon–Fri SGT</strong>.</p>
      <p>If your question is urgent, you can also reach us on WhatsApp during office hours.</p>
      <p style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #e3eae8; font-size: 13px; color: #5C6F6B;">
        This is an automated acknowledgement — please don't reply directly. A real human will follow up shortly.
      </p>
      <p style="font-size: 13px; color: #5C6F6B;">
        — The Superholic Lab team<br/>
        <a href="https://www.superholiclab.com" style="color: #B76E79; text-decoration: none;">superholiclab.com</a>
      </p>
    </div>
  `;
  return sendEmail({
    to: email,
    subject: `We've received your message — Superholic Lab`,
    html,
    replyTo: process.env.SUPPORT_INBOX_EMAIL || 'support@superholiclab.com'
  });
}

// ─── Internal alert to the support inbox so the team gets a poke
async function sendInternalAlert({ id, name, email, topic, message, routedTo }) {
  const html = `
    <div style="font-family: system-ui, sans-serif; max-width: 600px; line-height: 1.5;">
      <h2 style="margin: 0 0 12px 0;">New contact submission · #${id}</h2>
      <table style="border-collapse: collapse; font-size: 14px;">
        <tr><td style="padding: 4px 12px 4px 0; color: #5C6F6B;"><strong>From:</strong></td><td>${escapeHtml(name)} &lt;${escapeHtml(email)}&gt;</td></tr>
        <tr><td style="padding: 4px 12px 4px 0; color: #5C6F6B;"><strong>Topic:</strong></td><td>${escapeHtml(topic)}</td></tr>
        <tr><td style="padding: 4px 12px 4px 0; color: #5C6F6B;"><strong>Routed:</strong></td><td>${escapeHtml(routedTo)}</td></tr>
      </table>
      <hr style="border: none; border-top: 1px solid #e3eae8; margin: 16px 0;"/>
      <pre style="white-space: pre-wrap; font-family: 'SF Mono', Menlo, monospace; font-size: 13px; background: #f4fbf9; padding: 12px; border-radius: 6px;">${escapeHtml(message)}</pre>
      <p style="font-size: 13px; color: #5C6F6B; margin-top: 16px;">
        Reply directly to this email to respond — Reply-To is set to the submitter.
      </p>
    </div>
  `;
  return sendEmail({
    to: process.env.SUPPORT_INBOX_EMAIL || 'support@superholiclab.com',
    subject: `[${routedTo.toUpperCase()}] ${topic} — ${name}`,
    html,
    replyTo: email   // critical: hitting "Reply" goes back to the parent, not bouncing to no-reply
  });
}

// ─── Tiny HTML escape (we control the templates, but inputs come from users)
...
export async function handleQAQuestions(req, res) {
  ...
  // ── GET: list questions OR audit trail ─────────────────────────────────────
  ...
  // Params: q (full-text search), has_diagram=yes|no (visual_payload filter).
  // Sub-action ?action=audit_trail&id=<uuid> returns qa_reviews trail for one row.
  if (req.method === 'GET') {
    const urlParts = (req.url || '').split('?');
    const params   = new URLSearchParams(urlParts[1] || '');

    // ── Sub-action: audit trail for a single question ──
    if (params.get('action') === 'audit_trail') {
      const questionId = params.get('id');
      if (!questionId) return res.status(400).json({ error: 'id required' });
      try {
        const events = await readAuditTrail(adminDb, questionId);
        return res.status(200).json({ events, count: events.length });
      } catch (err) {
        console.error('[qa-questions] audit_trail error:', err.message);
  ...
  // ── PUT: update question ───────────────────────────────────────────────────
  if (req.method === 'PUT') {
    const { id, fields, approve, auditAction, auditNote } = req.body || {};
    ...
    const beforeSnap = await snapshotQuestion(adminDb, id);

    const { data: afterRow, error } = await adminDb
      .from('question_bank')
      .update(updates)
      .eq('id', id)
      .select('*')
    ...
    // Audit write — fire-and-forget, never blocks the response.
    // auditAction overrides inferred action (e.g. force 'rejected' from UI).
    const forced = auditAction
                 || (approve === true  ? 'approved'
                 :   approve === false ? 'rejected'
                 :   undefined);
    const inferred = inferActionFromDiff(beforeSnap, afterRow, forced);
    writeAuditEvent(adminDb, {
      questionId: id,
      reviewerId: user.id,
      action: inferred,
      ...
      before: beforeSnap,
      after: afterRow
    }).catch(err => console.error('[qa-questions] PUT audit failed:', err));
  ...
  if (req.method === 'DELETE') {
    const { id, auditNote } = req.body || {};
    if (!id) return res.status(400).json({ error: 'id is required' });

    // Snapshot before delete — qa_reviews FK is ON DELETE SET NULL,
    // so the audit row survives with the snapshot preserved.
    const beforeSnap = await snapshotQuestion(adminDb, id);
    ...
    writeAuditEvent(adminDb, {
      ...
      before: beforeSnap,
      after: null
    }).catch(err => console.error('[qa-questions] DELETE audit failed:', err));
...
export async function handleAutoFillQuestionBank(req, res) {
  ...
  try {
    ...
    const targetGaps = metrics || [
      ...
      { subject: 'English', level: 'Primary 4', topic: 'Cloze', type: 'cloze', difficulty: 'Advanced' }
    ];
...
export async function handleIngestBatchResults(req, res) {
  ...
  try {
    ...
    // 5. Filter against canonical taxonomy before insert
    const { accepted, rejected: canonRejected } = await filterByCanon(adminDb, newQuestions);
    if (canonRejected.length > 0) {
      console.warn(`[ingest-batch] Rejected ${canonRejected.length}/${newQuestions.length} off-canon questions:`, canonRejected.slice(0, 5));
    }

    // 6. Bulk Insert into Database (only canon-valid rows)
    if (accepted.length > 0) {
      const { error: insertErr } = await adminDb.from('question_bank').insert(accepted);
      if (insertErr) throw insertErr;
    }

    return res.status(200).json({
      success: true,
      questions_ingested: accepted.length,
      rejected_count: canonRejected.length,
      sample_rejections: canonRejected.slice(0, 5),
    });
...
async function advanceQuestStep(req, res, questId) {
  ...
  // Day 3 retries (after a `redo` outcome) hit advance-step a second time with the
  // same questId + step_index. The dedup key needs to differ across attempts so the
  // 2nd attempt isn't silently swallowed — append the score so a new attempt with
  // a different score gets fresh XP, while accidental double-submits with identical
  // score still dedupe.
  const eventIdSuffix = isLastStep ? `_${Math.round(Number(score ?? 0))}` : '';
  const xpResult = await grantXP(adminDb, {
    ...
    eventId:   `quest_step_${questId}_${step_index}${eventIdSuffix}`,
  });
...
async function day3Outcome(req, res, questId) {
  ...
  const adminDb = getAdminClient();
  const { data: quest, error: fetchErr } = await adminDb.from('remedial_quests')
    .select('student_id, subject, level, topic, status, steps, quest_title, day3_score, parent_quest_id, students!inner(parent_id)')
    .eq('id', questId)
    .single();
  if (fetchErr || !quest) return res.status(404).json({ error: 'Quest not found' });
  if (quest.students.parent_id !== user.id) return res.status(403).json({ error: 'Forbidden' });
  ...
  const now = new Date().toISOString();
  // All three outcomes close the original quest. Redo keeps the slot but
  // hands it to a freshly-spawned child quest with parent_quest_id lineage,
  // so the parent dashboard can trace the redo chain. (QUEST_PAGE_SPEC §15.)
  const updatePayload = {
  ...
  // Free the eligibility slot for non-redo outcomes. For redo we keep the
  // slot but reassign it below to the new child quest's id.
  if (outcome !== 'redo') {
    await adminDb.from('quest_eligibility')
      .delete()
      .eq('student_id', quest.student_id)
  ...
  if (outcome === 'redo') {
    // Spawn child quest. Steps copy verbatim from the original because the
    // topic + level haven't changed, so the difficulty bands are still
    // correct. Skipping the AI narrative call keeps the redo commit fast.
    const childTitle = `${(quest.quest_title || quest.topic).replace(/\s*\(Round \d+\)\s*$/, '').trim()} (Round 2)`;
    const { data: spawned, error: spawnErr } = await adminDb.from('remedial_quests').insert({
      student_id:         quest.student_id,
      subject:            quest.subject,
      level:              quest.level,
      topic:              quest.topic,
      trigger_score:      Math.round(quest.day3_score ?? 0),
      trigger_attempt_id: null,
      parent_quest_id:    questId,
      quest_title:        childTitle,
      steps:              quest.steps,
      current_step:       0,
      status:             'active',
    }).select('*').single();
    if (spawnErr) {
      console.error('[day3-outcome redo] spawn failed:', spawnErr.message || spawnErr);
      // Roll back the original close so the kid can retry from the modal.
      await adminDb.from('remedial_quests')
        .update({ day3_outcome: null, status: 'active', updated_at: now })
        .eq('id', questId);
      return res.status(500).json({ error: 'Failed to spawn redo quest. Please try again.' });
    }
    newQuest = spawned;

    // Reassign the eligibility slot to the child. Use upsert on the unique
    // (student_id, subject) PK so we overwrite the original's claim atomically.
    await adminDb.from('quest_eligibility').upsert({
      student_id: quest.student_id,
      subject:    quest.subject,
      quest_id:   spawned.id,
    }, { onConflict: 'student_id,subject' });

    // Growth-mindset bonus — +100 XP for choosing to try again. Emitted as a
    // 'quest_complete' event_type (within the existing CHECK constraint) but
    // tagged via metadata.bonus_type so analytics can split it out later.
    redoXp = await grantXP(adminDb, {
      studentId: quest.student_id,
      eventType: 'quest_complete',
      xpAmount:  100,
      metadata:  {
        bonus_type:       'redo_growth_mindset',
        parent_quest_id:  questId,
        child_quest_id:   spawned.id,
      },
      eventId:   `quest_redo_bonus_${questId}`,
    });
  }
  ...
  const badgesEarned = await evaluateBadges({
    studentId:     quest.student_id,
    eventType:     'quest_complete',
    eventMetadata: { outcome, quest_id: questId, parent_quest_id: quest.parent_quest_id, child_quest_id: newQuest?.id ?? null },
    db:            adminDb,
  });
...
// Maps the slug subject form ('english') to the canonical question_bank.subject value.
...
// How many student-facing graded items a single question_bank row produces.
// Cloze/editing rows expand into ~10 blanks/lines; comprehension into ~5
// sub-questions. The quest Day 1 target (12) is expressed in graded items,
// so for these types we fetch fewer outer rows and let the renderer expand.
function _subQuestionsPerItem(type) {
  if (type === 'cloze' || type === 'editing') return 10;
  if (type === 'comprehension') return 5;
  return 1;
}
...
async function questsBatch(req, res) {
  ...
  const baseTargetCount = stepCfg.question_count || (step_index === 0 ? 12 : 8);
  ...
  // Peek at one approved row to learn the dominant type, then scale the outer
  // row count for multi-sub-question types (editing/cloze/comprehension) so
  // the student-facing item count stays close to baseTargetCount.
  const { data: typeProbe } = await adminDb.from('question_bank')
    .select('type')
    ...
    .limit(1);
  const subPerItem  = _subQuestionsPerItem(typeProbe?.[0]?.type);
  const targetCount = subPerItem > 1
    ? Math.max(2, Math.ceil(baseTargetCount / subPerItem))
    : baseTargetCount;

  // Use stored bands or generate a sensible default
  const effectiveBands = bands.length > 0 ? bands : _defaultBands(dbLevel, baseTargetCount);
  const rawBandCounts = {};
  for (const b of effectiveBands) rawBandCounts[b] = (rawBandCounts[b] || 0) + 1;

  // Scale band counts proportionally for multi-sub-question types. Round to
  // integers and trim/pad so the totals match targetCount exactly.
  let bandCounts = rawBandCounts;
  if (subPerItem > 1) {
    bandCounts = {};
    for (const [band, count] of Object.entries(rawBandCounts)) {
      bandCounts[band] = Math.max(1, Math.round(count / subPerItem));
    }
    let total = Object.values(bandCounts).reduce((a, b) => a + b, 0);
    const order = Object.keys(bandCounts);
    while (total > targetCount && order.length > 0) {
      const k = order[order.length - 1];
      bandCounts[k] -= 1;
      total -= 1;
      if (bandCounts[k] <= 0) { delete bandCounts[k]; order.pop(); }
    }
    while (total < targetCount && order.length > 0) {
      bandCounts[order[0]] = (bandCounts[order[0]] || 0) + 1;
      total += 1;
    }
  }
  ...
  const questionSets = await Promise.all(
    Object.entries(bandCounts).map(async ([difficulty, count]) => {
      const { data } = await adminDb.from('question_bank')
        ...
        .not('approved_at', 'is', null)   // QA gate: only approved questions in quests
        .is('deprecated_at', null)
        .limit(count * 3);
