...
export async function handleGradeAnswer(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const { questionId, questionType, questionText, subject, studentAnswer, workedSolution, keywords, modelAnswer, parts, marks } = req.body || {};
    if (!questionType || studentAnswer === undefined || !questionText) return res.status(400).json({ error: 'Missing required fields' });
    if (!['word_problem','open_ended'].includes(questionType)) return res.status(400).json({ error: 'questionType must be word_problem or open_ended' });
