...
export async function handleGenerateQuestion(req, res) {
  ...
  try {
    ...
    let responseText, provider = 'openai-o4-mini';
    try { 
      const response = await openai.chat.completions.create({
        model: 'o4-mini',
        messages: [
          { role: 'system', content: GQ_SAFETY_PROMPT },
          { role: 'user', content: buildGQSchemaInstructions(subject, level, topic, type, difficulty, safeCount) }
        ],
        temperature: 0.4,
      });
      responseText = response.choices[0].message.content;
    } catch (e) { 
      console.error('o4-mini generation failed:', e);
...
async function executeGrading(systemPrompt, userPrompt) {
  const response = await openai.chat.completions.create({
    model: 'gpt-4o',
    messages: [
