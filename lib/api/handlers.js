...
export async function handleSummarizeChat(req, res) {
  ...
  try {
    ...
    const { data: student, error: stuErr } = await adminDb.from('students').select('parent_id').eq('id', student_id).single();
    if (stuErr || !student || student.parent_id !== user.id) {
    ...
    // Safely strip any potential backticks without regex
    const cleanedHtml = stripMarkdownFences(rawHtml);

    // 2. Generate a snappy title based on the note
    const rawTitle = await callGemini(`Based on this study note, generate a short, engaging title (max 40 chars). Return ONLY the title text, nothing else.\n\n${cleanedHtml.slice(0, 500)}`, {
      temperature: 0.4,
      maxOutputTokens: 30,
    });
    
    const cleanedTitle = stripMarkdownFences(rawTitle).replace(/["*]/g, '') || 'Miss Wena Study Note';

    // 3. Persist to Database
    const { data, error } = await adminDb.from('study_notes').insert({
      student_id,
      subject: (subject || 'general').toLowerCase(),
      topic: (topic || 'mixed').toLowerCase(),
      title: cleanedTitle,
    ...
    return res.status(500).json({ error: 'Could not generate study note at this time.' });
  }
}
