...
export async function handleCheckout(req, res) {
  ...
  try {
    ...
    // NOTE: Do NOT specify payment_method_types or automatic_payment_methods on
    // Checkout Sessions — those params are for PaymentIntents only.
    // Stripe Checkout automatically presents all payment methods configured in
    // your Stripe Dashboard > Settings > Payment methods (card, PayNow, etc.).
    const sessionParams = {
      mode:               'subscription',
    ...
    // Only add customer_email if present — adding it when a Stripe customer
    // already exists for that email can cause a conflict error.
