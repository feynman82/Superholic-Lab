...
// ─── PRICING UTILITIES ────────────────────────────────────────────────────────
// MRR: Singapore FRS 115 — annual recognised at 1/12 per month.
...
export async function handleChat(req, res) {
  ...
  const cleanHistory = messages.filter(m => m.role === 'user' || m.role === 'assistant').slice(-MAX_HISTORY_MESSAGES).map(m => {
    const parts = [];
...
function buildGenerateUserPrompt(type, count, subject, level, topic, difficulty) {
  ...
  const extras = {
    ...
    open_ended:   `\n- Root 'passage' (experiment setup). A 'parts' array of sub-questions using part_type: 'text_box' or 'true_false'.`,
    ...
    comprehension:`\n- Root 'passage' (300+ words). A 'parts' array of sub-questions using part_type: 'mcq', 'true_false', 'referent', 'sequencing', or 'text_box'.`,
...
function buildGradingRubric(questionType, subject, studentAnswer, question) {
  ...
  if (questionType === 'comprehension') return `\nQUESTION (${marks} marks): ${questionText}\n\nMODEL ANSWER: ${modelAnswer||workedSolution||'N/A'}\nGRADING RUBRIC: ${rubric||'Award marks based on semantic similarity.'}\nSTUDENT ANSWER: ${studentAnswer}\n\nTotal: ${marks}mk`;
  if (questionType === 'open_ended' && subject === 'Science') return `\nQUESTION (${marks} marks): ${questionText}\n\nKEYWORDS: ${(keywords||[]).join(', ')}\nMODEL ANSWER: ${modelAnswer||workedSolution}\nSTUDENT ANSWER: ${studentAnswer}\n\nRUBRIC (CER): Claim 1mk, Evidence 1mk, Reasoning 1+mk. Total: ${marks}mk`;
  if (questionType === 'word_problem' && subject === 'Mathematics') { const partsText = (parts||[]).map(p => `${p.label}: ${p.question} → ${p.correct_answer}`).join('\n'); return `\nQUESTION (${marks} marks): ${questionText}\n\nPARTS:\n${partsText}\n\nWORKED: ${workedSolution}\nSTUDENT: ${studentAnswer}\n\nRUBRIC: 1mk method + 1mk answer per part. Total: ${marks}mk`; }
...
export async function handleGenerateQuest(req, res) {
  ...
  try {
    ...
    // 🚀 MASTERCLASS: Deterministic Logic Engine (0% Route Failure Rate)
    const deterministicSteps = [
      {
        day: 1,
        title: "Diagnostic Briefing",
        type: "tutor",
        action_url: `/pages/tutor.html?subject=${safeSubject}&level=${safeLevel}&intent=remedial&topic=${safeTopic}`,
        status: "pending"
      },
      {
        day: 2,
        ...
        type: "quiz",
        action_url: `/pages/quiz.html?subject=${safeSubject}&level=${safeLevel}&topic=${safeTopic}&difficulty=Foundation`,
        status: "locked"
      },
      {
        day: 3,
        title: "Mastery Trial",
        type: "quiz",
        action_url: `/pages/quiz.html?subject=${safeSubject}&level=${safeLevel}&topic=${safeTopic}&difficulty=Standard`,
        status: "locked"
      }
    ];

    // AI is restricted to generating narrative text only
    ...
    {
      "quest_title": "Creative Title (e.g., The Fraction Core Rescue!)",
      "day_1_desc": "Brief engaging description for Day 1 (Tutor session)",
      "day_2_desc": "Brief engaging description for Day 2 (Foundation practice)",
      "day_3_desc": "Brief engaging description for Day 3 (Mastery trial)"
    }`;

    let questData;
    try {
      // Force JSON mimetype output via Gemini 1.5 Flash for high-speed generation
      const raw = await callGemini(`You are a creative educational writer for Singapore primary students. Return ONLY valid JSON.\n\n${narrativePrompt}`, { temperature: 0.7, maxOutputTokens: 1024, responseMimeType: 'application/json' });
      const parsedNarrative = JSON.parse(stripMarkdownFences(raw).replace(/[\n\r]/g, ' '));
      
      deterministicSteps[0].description = parsedNarrative.day_1_desc;
      deterministicSteps[1].description = parsedNarrative.day_2_desc;
      deterministicSteps[2].description = parsedNarrative.day_3_desc;
      
      questData = { quest_title: parsedNarrative.quest_title, steps: deterministicSteps };
    } catch (e) {
      console.error("[generate-quest] Narrative generation fallback triggered:", e.message);
      // Failsafe: If AI hallucinates JSON or API times out, fallback to static text so the student is never blocked.
      deterministicSteps[0].description = `Review ${topic} concepts with Miss Wena to uncover your blind spots.`;
      deterministicSteps[1].description = `Practice foundational questions to stabilize your understanding.`;
      deterministicSteps[2].description = `Take the final trial to prove your mastery of ${topic}.`;
      questData = { quest_title: `${topic} Recovery Quest`, steps: deterministicSteps };
    }
...
export async function handleAdmin(req, res) {
  ...
  if (req.method === 'GET') {
    const [profilesRes, studentsRes, subsRes, recentActivityRes, lastActRes, contactRes] = await Promise.all([
      ...
      adminDb.from('students')
        ...
        .order('created_at', { ascending: false }),
      // created_at included so admin.html can compute last-7-days new subscriber feed
      ...
      // Count unread contact form submissions — table: contact_submissions, status: 'pending'
    ...
    return res.status(200).json({
      ...
      // contact_pending: count of unread contact submissions — shown in admin Business Pulse
...
export async function handleAdminEdit(req, res) {
  ...
  if (action === 'update_parent') {
    ...
    // Email change triggers confirmation to the new address
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLER: /api/contact
// Stores contact form submissions from /pages/contact.html into Supabase.
// Public endpoint — no auth required.
// Table: contact_submissions (migration 012)
// Status values: 'pending' (default) | 'replied'
// Admin reads pending count via /api/admin → contact_pending field.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function handleContact(req, res) {
  ...
  try {
    ...
    const { error } = await adminDb.from('contact_submissions').insert({
      ...
      topic:   (topic || 'General Inquiry').trim().slice(0, 100),
