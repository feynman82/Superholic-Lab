...
function buildGenerateUserPrompt(type, count, subject, level, topic, difficulty) {
  ...
  const extras = {
    ...
    open_ended:   `\n- Model answer in paragraph form, keywords list, 2/1/0 marking rubric.`,
    ...
    comprehension:`\n- Root 'passage' (300+ words). A 'parts' array of sub-questions using part_type: 'mcq', 'true_false', 'referent', 'sequencing', or 'text_box'. Double-escape all SQL quotes.`,
  };
...
export async function handleGenerate(req, res) {
  ...
  const GENERATE_PROMPTS = { mcq: MCQ_SYSTEM_PROMPT, short_ans: SHORT_ANS_SYSTEM_PROMPT, word_problem: WORD_PROBLEM_SYSTEM_PROMPT, open_ended: OPEN_ENDED_SYSTEM_PROMPT, cloze: CLOZE_SYSTEM_PROMPT, editing: EDITING_SYSTEM_PROMPT, comprehension: OPEN_ENDED_SYSTEM_PROMPT };
  const GENERATE_MAX_COUNT = { mcq: 10, short_ans: 10, word_problem: 5, open_ended: 10, cloze: 3, editing: 3, comprehension: 2 };
...
function buildGQSchemaInstructions(subject, level, topic, type, difficulty, count) {
  ...
  const schemas = {
    ...
    open_ended:   `\n- keywords: 3-5 mark-bearing terms\n- model_answer: CER structure string\n`,
...
export async function handleGenerateQuestion(req, res) {
  ...
  try {
    ...
    if (!['mcq','short_ans','word_problem','open_ended','cloze','editing'].includes(type)) return res.status(400).json({ error: 'Invalid type' });
...
export async function handleGradeAnswer(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const { questionId, questionType, questionText, subject, studentAnswer, workedSolution, keywords, modelAnswer, parts, marks, rubric } = req.body || {};
    if (!questionType || studentAnswer === undefined || !questionText) return res.status(400).json({ error: 'Missing required fields' });
    if (!['word_problem','open_ended','comprehension'].includes(questionType)) return res.status(400).json({ error: 'Invalid questionType' });
