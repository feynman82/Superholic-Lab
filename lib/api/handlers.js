...
export async function handleChat(req, res) {
  ...
  // RAP must NEVER surface errors to the student. Skipped entirely if quest
  // mode already overrode the prompt (Quest Day-2 Socratic flow is Sprint 5).
  //
  // Sprint 4 additions: canary rollout via WENA_RAP_ROLLOUT_PCT, CHECK_RESPONSE
  // mode detection, and per-turn telemetry written after the AI reply.
  ...
  if (RAP_ENABLED && systemPrompt === OMNI_TUTOR_SYSTEM_PROMPT) {
    try {
      const subjectN  = _normaliseSubject(rapSubject);
      const levelN    = _normaliseLevel(rapRawLevel);

      if (subjectN === 'English' && levelN && rapTopicIn) {
        await loadServerPlaybook();
        // Exact match first; topic-level fallback if sub_topic absent or unmatched.
        ...
        // Sprint 4: if the previous assistant turn was DIRECT_TEACH and ended
        // with the cell's check question, this user turn is a CHECK_RESPONSE.
        let mode = baseMode;
        ...
        // For CHECK_RESPONSE turns, the question being answered is the cell's
        // check question (asked at the close of the previous DIRECT_TEACH turn);
        // otherwise the client may pass an explicit override via lastCheckQuestion.
        ...
        // a validation throw must not leak partial state into telemetry.
        ...
        wenaRapFired        = true;
        console.log(
          `[wena-rap] mode=${mode} level=${levelN} topic=${rapTopicIn} sub_topic=${rapSubTopicIn || 'n/a'} cell=${cell ? wenaCellHitKind : 'no'} version=${WENA_PROMPT_VERSION}`
        );
      } else if (subjectN === 'English') {
      ...
      // For Maths/Science/general: silently skip RAP — wenaFallbackReason set above.
    } catch (rapErr) {
      console.error('[wena-rap] prompt-assembly error, falling back to legacy:', rapErr?.message || rapErr);
      systemPrompt        = OMNI_TUTOR_SYSTEM_PROMPT;  // safety: defensive reset
    ...
    // RAP enabled but quest mode already set its own prompt — Sprint 5 territory.
