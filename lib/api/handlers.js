...
1. The 3-Strike Scaffolding Rule: NEVER give the final answer immediately. Guide the student step-by-step. If they fail or guess incorrectly 3 times in a row, you MUST:
   - Provide the complete, clear worked solution.
   - Explain the core concept they missed.
   - Generate a NEW, slightly EASIER "confidence builder" question on the exact same concept so they can secure a win and rebuild their confidence.
2. The "Zero-Effort" Protocol: If a student replies with zero effort (e.g., "idk", "just tell me", or a blank response) during scaffolding, DO NOT give the answer. Instead, give them a gentle, multiple-choice/binary hint to force engagement. (e.g., "It's okay to be stuck! Do you think the answer will be bigger or smaller than the original number?").
3. The Soft Pivot (Guardrails): If the student asks about non-academic topics (e.g., Batman, video games), humorously acknowledge it, but firmly pivot back. (e.g., "Batman is great, but right now we need to be superheroes at Fractions! Let's look at..."). Do not entertain off-topic discussions.
4. The Signature Celebration: When the student finally gets the answer right (on their own or on a retry question), you MUST celebrate enthusiastically and gamify the moment: "BOOM! 🌟 You totally nailed it! I knew you could figure it out. How does your brain feel? Ready to level up to the next one?"
5. Visual Analysis: If the student shares a drawing or image of their working, analyze it carefully to spot exactly where their logic broke down. Point out their specific mistake gently.
6. MOE Alignment: Use exact Singapore syllabus terminology (e.g., CER for Science, CRA/Bar Models for Maths, Grammar Rules for English). Use **bold** text for key MOE terminology.
7. Tone: Warm, cheerful, highly empathetic, using very light Singlish ("lah", "lor") occasionally to sound natural.`;
...
2. Underlying Concept: Name the exact MOE-aligned concept.
3. Step-by-Step Breakdown: Map out the logic and steps discussed.
4. Miss Wena's Key Takeaway: A final encouraging tip.
...
OUTPUT FORMAT:
You MUST return ONLY a valid JSON object with this exact structure. Do NOT wrap the response in markdown fences (no \`\`\`json):
{
  "title": "<A short 3-6 word title for this study note>",
  "content_html": "<The generated HTML string>"
}`;
...
export async function handleSummarizeChat(req, res) {
  ...
  try {
    ...
    let generatedData;
    try {
      const cleanedJson = stripMarkdownFences(rawResponse);
      generatedData = JSON.parse(cleanedJson);
    } catch (parseErr) {
      console.error('[summarize-chat] ERROR: JSON Parse Failed. Raw String:', rawResponse);
      throw new Error('AI returned malformed JSON.');
    }

    if (!generatedData.title || !generatedData.content_html) {
      console.error('[summarize-chat] ERROR: Missing schema keys in JSON:', generatedData);
      throw new Error('AI failed to generate required schema properties.');
    }
    ...
    if (dbErr) {
      console.error('[summarize-chat] ERROR: Supabase Insert Failed:', dbErr);
      throw dbErr;
    }
    ...
    console.error('[summarize-chat] CRITICAL ERROR:', err.message || err);
...
/**
 * Returns the Stripe price ID map from environment variables.
 * Called at request time so env vars are always current.
 */
...
/**
 * Builds a reverse map of priceId → planName for webhook lookups.
 */
function getPriceToPlan() {
  const prices = getStripePrices();
  return Object.fromEntries(
    Object.values(prices).flatMap(plans =>
      Object.entries(plans)
        .filter(([, priceId]) => !!priceId)
        .map(([planName, priceId]) => [priceId, planName])
    )
  );
}
 ...
 * Accepts 'yearly' (frontend) or 'annual' (internal) as equivalent.
 */
...
export async function handleCheckout(req, res) {
  ...
  if (!priceId) {
    const cycle = (billing === 'annual' || billing === 'yearly') ? 'annual' : 'monthly';
    console.error(`[checkout] Missing price ID for plan=${plan} billing=${cycle}. Check STRIPE_*_PRICE_ID env vars.`);
...
export async function handleGradeAnswer(req, res) {
  ...
  try {
    ...
    let responseText;
    let gradedBy = 'claude'; // 🌟 MASTERCLASS FIX: Claude is now the primary grader

    try {
      // 1. Call Claude 3.5 Sonnet (Best at strict logical adherence & JSON generation)
      responseText = await callClaudeRaw(GRADER_SYSTEM_PROMPT, rubricPrompt, { 
        maxTokens: 1024, 
        model: 'claude-3-5-sonnet-20241022' 
      });
    } catch (claudeErr) {
      console.warn('[grade-answer] Claude failed, falling back to Gemini:', claudeErr.message);
      gradedBy = 'gemini';
      // 2. Fallback to Gemini if Anthropic API is down
...
const QUEST_SYSTEM_PROMPT = `You are a gamified, highly engaging educational AI creating 
3-day "Plan Quests" for primary school students on Superholic Lab.

NARRATIVE TONE (Epic & Gamified):
Make the quest sound like an exciting mission! Use emojis, action words, and speak directly to the student.
Example Title: "Operation: Fractions Rescue! 🚀"
Example Day 1: "Team up with Miss Wena to crack the denominator code!"

PEDAGOGICAL ARC (The Confidence Staircase):
- Day 1 MUST be type "tutor". Focus: Rebuilding core concepts with Miss Wena.
- Day 2 MUST be type "quiz" (MCQ only). Focus: Securing an easy win (Foundation difficulty).
- Day 3 MUST be type "quiz" (Short Answer or Word Problem). Focus: Testing true mastery (Standard/Advanced).

URL RULES (Strict!):
- Day 1 (Tutor): /pages/tutor.html?subject={subject}&level={level}&intent=remedial&topic={topic}&score={score}
- Day 2 (Quiz): /pages/quiz.html?subject={subject}&level={level}&topic={topic}&type=mcq
- Day 3 (Quiz): /pages/quiz.html?subject={subject}&level={level}&topic={topic}&type=short_ans
All URL values must use lowercase hyphenated slugs. Level format: "primary-4".

OUTPUT FORMAT:
You MUST return ONLY a valid JSON object matching this exact schema. Do NOT wrap in markdown fences.
{
  "quest_title": "<String, Max 60 chars>",
  "steps": [
    {
      ...
      "action_url": "<String matching URL RULES>",
      "estimated_minutes": <Integer 5 to 30>
    }
  ]
}`;
...
export async function handleGenerateQuest(req, res) {
  ...
  try {
    ...
    const userPrompt = `Generate an epic 3-day remedial quest for ${student.name || 'a student'} (${student.level || level}).
    They scored ${Math.round(trigger_score)}% (${scoreLabel}) on the topic "${topic}" in ${subject}.
    subject slug: ${safeSubject} | level slug: ${safeLevel} | topic slug: ${safeTopic} | score: ${Math.round(trigger_score)}
    Day 1 action_url MUST EXACTLY BE: /pages/tutor.html?subject=${safeSubject}&level=${safeLevel}&intent=remedial&topic=${safeTopic}&score=${Math.round(trigger_score)}
    
    CRITICAL JSON RULE: Do NOT use any raw line breaks or newlines inside the JSON string values. Keep all text on a single line!`;

    let questData;
    
    try {
      const rawResponse = await callGemini(`${QUEST_SYSTEM_PROMPT}\n\nUSER PROMPT:\n${userPrompt}`, {
        temperature: 0.4,
        maxOutputTokens: 1024,
        responseMimeType: "application/json"
      });
      ...
      // THE SHIELD: Strip all raw newlines to prevent "Unterminated string" JSON parse crashes
      const safeJson = cleanedJson.replace(/[\n\r]/g, ' '); 
      questData = JSON.parse(safeJson);
      
    } catch (geminiErr) {
      console.warn('[generate-quest] Gemini JSON failed, falling back to Claude:', geminiErr.message);
      const rawResponse = await callClaudeRaw(QUEST_SYSTEM_PROMPT, userPrompt, { maxTokens: 1024 });
...
// Generates a full exam paper for a given template key.
// Uses Gemini primary → Claude Haiku fallback, same pattern as generate-question.
// Returns: { templateKey, sections, totalMarks, durationMinutes, generatedAt }
// ─────────────────────────────────────────────────────────────────────────────

const GE_SYSTEM_PROMPT = `You are a Singapore MOE-aligned exam paper generator.
Generate exam questions for the requested subject, level, and section.
Return ONLY a JSON array of question objects. No markdown, no commentary.
Each question must follow the Superholic Lab question schema exactly.
All questions must use Singapore context (SG names, locations, food, currency SGD).
Questions must be age-appropriate and curriculum-aligned for the target Primary level.`;

/**
 * Builds a prompt to generate questions for a single exam section.
 *
 * @param {object} section  - Section descriptor from an exam template
 * @param {string} subject  - 'Mathematics' | 'Science' | 'English'
 * @param {string} level    - 'Primary 3' through 'Primary 6'
 * @param {number} count    - Number of questions to generate
 * @returns {string}
 */
function buildGEPrompt(section, subject, level, count) {
  const typeInstructions = {
    mcq:          'MCQ with 4 options (A/B/C/D), correct_answer letter, and wrong_explanations for each wrong option.',
    short_ans:    'Short answer with correct_answer string and optional accept_also array.',
    word_problem: 'Word problem with 2-3 parts, each with label, question, marks, correct_answer, and worked_solution.',
    open_ended:   'Open-ended question with keywords array and model_answer following CER framework.',
    cloze:        'Cloze passage with [N] blank tokens and blanks array (4 options each, correct_answer letter).',
    editing:      'Editing passage_lines with exactly 1 error line (has_error: true) and correct_word.',
  };
...
Marks per question: ${section.marksPerQuestion || 1}
${section.instructions ? `Instructions context: ${section.instructions}` : ''}

Format for each question:
${typeInstructions[section.questionType] || 'Follow the standard question schema.'}

Required base fields for every question:
- id: unique string like "p5m-ge-001"
- subject: "${subject}"
- level: "${level}"
- topic: (relevant topic for this section)
- sub_topic: (specific concept)
- difficulty: one of "Foundation", "Standard", "Advanced", "HOTS"
- type: "${section.questionType}"
- marks: ${section.marksPerQuestion || 1}
- question_text: the question
- worked_solution: minimum 3 numbered steps

Return ONLY a JSON array of ${count} question objects.`;
}
...
export async function handleGenerateExam(req, res) {
  ...
  try {
    const body = req.body || {};
    const { templateKey, subject, level, sections: requestedSections } = body;

    if (!templateKey || !subject || !level) {
      return res.status(400).json({ error: 'Missing required fields: templateKey, subject, level' });
    }
    ...
    const validLevels = ['Primary 3', 'Primary 4', 'Primary 5', 'Primary 6'];
    if (!validLevels.includes(level)) {
      return res.status(400).json({ error: 'Invalid level — must be "Primary 3" through "Primary 6"' });
    }

    // Sections to generate — caller can pass a subset, or we generate all
    const sectionsToGenerate = Array.isArray(requestedSections) && requestedSections.length > 0
      ? requestedSections
      : null;  // null means "generate a default set"

    // If no sections provided, generate a minimal set (API returns questions for rendering)
    // Callers with the full template can pass sections from getTemplate(templateKey).sections
    const sections = sectionsToGenerate || [
      { label: 'Section A', title: 'Multiple Choice', questionType: 'mcq',  marksPerQuestion: 2, questionCount: 5 },
    ];
    ...
    for (const sec of sections) {
      ...
      const prompt = GE_SYSTEM_PROMPT + '\n\n' + buildGEPrompt(sec, subject, level, count);
      ...
      try {
        ...
        console.warn('[generate-exam] Gemini failed for section, falling back to Claude:', geminiErr.message);
        provider     = 'claude';
        responseText = await callClaudeRaw(
          GE_SYSTEM_PROMPT,
          buildGEPrompt(sec, subject, level, count),
      ...
      generatedSections.push({
        ...
        sectionMarks,
        questions:    questions.map(q => ({ ...q, source: 'ai', aiProvider: provider })),
      });
    }
    ...
    return res.status(500).json({ error: 'Failed to generate exam paper. Please try again.' });
  }
}
...
export async function handlePortal(req, res) {
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    ...
    return res.status(500).json({ error: 'Billing portal is not configured. Please contact support.' });
  }
  ...
  try {
    ...
    const session = await stripe.billingPortal.sessions.create({
      ...
      return_url: `${appUrl}/pages/dashboard.html`,
    });
  ...
  // TEST: POST /api/portal with a valid Authorization header.
  // Expects { url } pointing to Stripe's hosted portal.
  // Without a stripe_customer_id on the profile, expects 400.
}