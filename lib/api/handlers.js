...
${convo}`;
    ...
    // SAFE REGEX: Using \`{3}\` instead of three backticks to prevent parser crashes
    const cleanHtml = htmlContent.replace(/^`{3}(?:html)?\n?/m, '').replace(/\n?`{3}$/m, '').trim();
    ...
    // SAFE REGEX for title parsing
    const cleanTitle = title.replace(/["']/g, '').replace(/^`{3}(?:text)?\n?/m, '').replace(/\n?`{3}$/m, '').trim();
