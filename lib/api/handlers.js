...
export async function handleSummarizeChat(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.error('[summarize-chat] JSON parse failed:', rawResponse);
...
export async function handleCheckout(req, res) {
  ...
  try {
    // Use the latest Stripe API version supported by SDK v17
    const stripe = new Stripe(process.env.STRIPE_SECRET_KEY, { apiVersion: '2025-01-27.acacia' });

    const sessionParams = {
      mode:        'subscription',
      line_items:  [{ price: priceId, quantity: 1 }],
      // automatic_payment_methods replaces the deprecated payment_method_types: ['card']
      // It enables all payment methods configured in your Stripe dashboard (card, PayNow etc)
    ...
    // Only add customer_email if we have one — avoids conflict if customer already exists in Stripe
    ...
    // Surface the real Stripe error so it shows in the browser during testing.
    // The err.message from Stripe is always safe to return (no secrets).
...
export async function handleWebhook(req, res) {
  ...
  if (!process.env.STRIPE_WEBHOOK_SECRET) {
    // Webhook secret not configured yet — log and return 200 so Stripe stops retrying
    console.warn('[webhook] STRIPE_WEBHOOK_SECRET not set. Webhook received but not verified.');
  ...
  try {
    switch (event.type) {
      ...
      case 'invoice.payment_failed': {
        ...
        // Find subscription and mark as past_due — don't cut access yet
...
export async function handleGradeAnswer(req, res) {
  ...
  try {
    ...
    try {
      responseText = await callClaudeRaw(GRADER_SYSTEM_PROMPT, rubricPrompt, { maxTokens: 1024, model: 'claude-3-5-sonnet-20241022' });
    } catch (claudeErr) {
...
export async function handleSaveExamResult(req, res) {
  ...
  try {
    ...
    const { data, error } = await adminDb.from('exam_results').insert({
...
export async function handleGenerateQuest(req, res) {
  ...
  try {
    ...
    try {
      ...
      const safeJson = stripMarkdownFences(rawResponse).replace(/[\n\r]/g, ' ');
...
Given a list of recently failed quiz questions, synthesise them into ONE root conceptual gap — the deepest underlying cause, not a surface description.
  ...
  try {
    ...
    if (result.quest?.quest_title && Array.isArray(result.quest?.steps) && result.quest.steps.length === 3) {
      const { data: inserted, error: insertErr } = await adminDb.from('remedial_quests').insert({
        ...
        trigger_score: null, trigger_attempt_id: null,
...
export async function handleGenerateExam(req, res) {
  ...
  try {
    ...
    if (!validLevels.includes(level)) return res.status(400).json({ error: 'Invalid level — must be "Primary 3" through "Primary 6"' });

    const sections = (Array.isArray(requestedSections) && requestedSections.length > 0)
      ? requestedSections
    ...
    for (const sec of sections) {
      ...
      try {
        responseText = await callGemini(prompt, { temperature: 0.45, maxOutputTokens: 4096, responseMimeType: 'application/json' });
      } catch (geminiErr) {
        console.warn('[generate-exam] Gemini failed, falling back to Claude:', geminiErr.message);
        provider     = 'claude';
