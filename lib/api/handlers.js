...
export async function handleSummarizeChat(req, res) {
  ...
  try {
    ...
    if (!student_id || !messages || messages.length === 0) {
    ...
    const prompt = `Summarise this tutoring session into a concise, encouraging study note for a primary school student.
Use rich HTML (e.g. <ul>, <strong>, <h3>) but NO markdown fences. Do not include CSS or classes.
Keep it under 250 words. Focus on the core concept discussed.
Conversation:
${convo}`;

    // Generate the note
    const htmlContent = await callGemini(prompt, { temperature: 0.3 });
    
    // Safely strip markdown formatting without breaking syntax parsers
    const startFence = new RegExp('^`{3}(?:html)?\\n?', 'm');
    const endFence = new RegExp('\\n?`{3}$', 'm');
    const cleanHtml = htmlContent.replace(startFence, '').replace(endFence, '').trim();

    // Generate a smart title
    const titlePrompt = `Write a short 3-6 word title for this study note based on this content:\n${cleanHtml}`;
    const title = await callGemini(titlePrompt, { temperature: 0.3 });
    
    const titleStart = new RegExp('^`{3}(?:text)?\\n?', 'm');
    const cleanTitle = title.replace(/["']/g, '').replace(titleStart, '').replace(endFence, '').trim();
