...
export async function handleGradeAnswer(req, res) {
  ...
  try {
    ...
    if (!['word_problem','open_ended','comprehension','visual_text'].includes(questionType)) return res.status(400).json({ error: 'Invalid questionType' });
  ...
  if (req.method === 'GET') {
    const [profilesRes, studentsRes, subsRes, recentActivityRes, lastActRes] = await Promise.all([
      ...
      adminDb.from('students')
        ...
        .order('created_at', { ascending: false }),
      // created_at included so admin.html can filter for last-7-days new subscriber feed
      adminDb.from('subscriptions')
        .select('id, profile_id, stripe_subscription_id, stripe_price_id, plan_name, billing_cycle, status, current_period_end, paused_at, resumes_at, created_at')
...
export async function handleExport(req, res) {
  ...
  try {
    ...
    const [profileRes, subsRes, quizRes, referralRes] = await Promise.all([
      ...
      studentIds.length > 0
        ? adminDb.from('quiz_attempts').select('subject, level, topic, score, total_questions, completed_at').in('student_id', studentIds).order('completed_at', { ascending: false }).limit(500)
...
export async function handleAdminEdit(req, res) {
  ...
  if (action === 'update_parent') {
    ...
    // Email change triggers confirmation to new address (not a silent override)
    if (email?.trim()) {
      const { error: authErr } = await adminDb.auth.admin.updateUserById(userId, { email: email.trim() });
      if (authErr) console.error('[admin-edit] Auth email update error:', authErr.message);
    }
