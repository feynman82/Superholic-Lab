    ...
    const openAiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert Singapore MOE PSLE Examiner. Grade the student's answer strictly based on the provided Model Answer/Worked Solution. Output ONLY a JSON object with this exact structure: {"score": <integer between 0 and max marks>, "feedback": "<1-2 short sentences of constructive feedback explaining the score>"}. Do not include markdown formatting.`
          },
          {
            role: "user",
...
export async function handleAnalyzeWeakness(req, res) {
  ...
  try {
    ...
    const { data: attempts, error } = await db
      ...
      .select('topic, cognitive_skill, correct, marks_earned, marks_total')
    ...
    attempts.forEach(att => {
      if (!masteryScores[att.topic]) {
        masteryScores[att.topic] = { totalWeight: 0, correctWeight: 0, failedSkills: new Set() };
      }
      const isHOTS = att.cognitive_skill === 'HOTS';
      const weight = isHOTS ? 1.5 : 1.0;
      
      let isCorrect = att.correct;
      if (att.marks_total > 0) {
        isCorrect = (att.marks_earned / att.marks_total) >= 0.5; 
      }

      masteryScores[att.topic].totalWeight += weight;
      if (isCorrect) {
        masteryScores[att.topic].correctWeight += weight;
      } else if (att.cognitive_skill) {
        masteryScores[att.topic].failedSkills.add(att.cognitive_skill);
      }
    });
    ...
    return res.status(200).json({
      ...
      ai_prompt_context: `Student is struggling with ${rootCauseTopic}. Symptoms observed in ${weakestTopic}.`
    });
